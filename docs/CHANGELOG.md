...
Stage 6, PR 23 of `docs/superpowers/specs/2026-09-03-multi-window-tabs-plan.md` — "A real native
Save/Open dialog (and write-back-in-place) where `VW.capabilities.fileSystemAccess` is true; the
existing blob/`<a download>` path (PR 3) stays as the universal fallback, never removed." Depends on
PR 3 (`exportFile`/`importFile`, already merged) and PR 19 (`VW.capabilities.fileSystemAccess`,
already merged) — both reused directly, never re-derived.

**Two new functions on `VW.workspace`, both gated on the SAME `_capabilities.fileSystemAccess` PR 19
already established:**
- **`exportFileNative(id) -> Promise<boolean>`.** Where the capability is true: calls
  `window.showSaveFilePicker()` (a suggested filename derived from the workspace's own name plus
  `.json`, a JSON `types` filter) directly inside the click handler that triggers it, writes the
  ...
  this-tab-only, in-memory map keyed by workspace id — never persisted to localStorage/IndexedDB (a
  handle cannot be trivially serialized there; a real IndexedDB-handle-persistence scheme exists in
  principle but is meaningfully more complex and not required by the plan, stated plainly rather
  than attempted). Where the capability is false: performs the exact same
  Blob/`URL.createObjectURL`/`<a download>` trigger `workspaces.html`'s own `downloadFile()` already
  uses, mirrored byte-for-byte — one function call gets the best available behavior regardless of
  tier, resolving `true` once the download was triggered (there is no way to know if the technician
  actually kept it, an honest, stated limitation of that path only).
- **`importFileNative() -> Promise<string|null>`.** Only meaningful where the capability is true —
  when false, rejects clearly with a specific message rather than building a second, redundant
  fallback (the existing `<input type="file">` UI already IS the complete, correct fallback for
  opening a file on that tier). Calls `window.showOpenFilePicker()` (same JSON filter), reads the
  picked file's text via the handle's `getFile()`/`.text()`, feeds it through the EXISTING
  `_wsImportFromJson()` — the same PR 3/PR 22 validation/migration path, never a second,
...
design doc names it for:** a second `exportFileNative()` call for an id already in the in-memory
map re-verifies write permission via the handle's own `queryPermission()`/`requestPermission()`
methods FIRST — never assumes a remembered handle is still writable, since permission can be
revoked by the user or the browser between calls — and only reuses it (no new picker shown) once
that check genuinely confirms `"granted"`. A revoked/denied handle is dropped and this falls
through to a fresh `showSaveFilePicker()` call, exactly as if nothing had ever been remembered for
that id.

**CANCEL IS NOT AN ERROR:** a technician dismissing either native picker throws a real
`DOMException` named `"AbortError"` — a normal, expected choice, never a failure. Both functions
catch specifically that case and resolve (never reject) `false`/`null` respectively; any OTHER
rejection (a genuine I/O failure, a permission the browser refuses outright) still propagates as a
real rejection, proven separately so "cancel never rejects" cannot be mistaken for "nothing ever
rejects".

**UI (`engine/ui/workspaces.html`):** a feature-detected "💾 Save to file…" button ADDED next to
every workspace row's existing "⬇ Download .json" button, and a feature-detected "🗂 Open from
file…" button ADDED next to the existing "📁 Import from file…" control — both visible/enabled
only when `VW.capabilities.fileSystemAccess` is true, both calling the two new functions above. The
existing download link, `<input type="file">` pair, and their click handlers are completely
untouched — every technician on a tier without this capability keeps exactly today's behavior, with
nothing removed or modified.

**Placement:** per the by-now well-established `test_a2_popout.py` cross-PR coupling hazard, all new
code lands after PR 3's `workspaceImportFile()` and well before `popoutControl()`'s own section,
...
own") plus two `node --check` syntax gates and the node suite's own rollup (52/0); that node suite
itself carries **52 real assertions** against the real exported `VW.workspace` functions, with a
hand-rolled mock `showSaveFilePicker`/`showOpenFilePicker`/`FileSystemFileHandle` (permission
state fully scriptable, including changing it BETWEEN two calls to simulate a revoked grant): the
fallback path proven against a real Node `Blob` (mocked `URL.createObjectURL` receiving a Blob whose
text is exactly `_wsExportPayload()`'s own JSON); the native path's `showSaveFilePicker()` options
(suggested filename, JSON type filter) and the exact JSON written via the mocked writable stream;
write-back-in-place proven with a call-count spy (a second `exportFileNative()` call for the same id
never calls `showSaveFilePicker()` again); a revoked/denied permission on a previously-granted
handle proven to be re-checked and to fall back to a genuinely fresh picker call rather than
silently reusing or failing; cancel-is-not-an-error proven in both directions, contrasted directly
against a genuine (non-abort) rejection still propagating; `importFileNative()` proven to route
through the EXISTING `_wsImportFromJson()` by feeding it a `schemaVersion: 999` payload and
confirming the SAME "Workspace import failed: schemaVersion 999 is newer than this app version
understands…" message PR 22's own classifier produces (not a second, hand-rolled comparison); a
successful import's handle proven remembered by a subsequent `exportFileNative()` call for that
same id never re-prompting; and `importFileNative()` proven to reject clearly (never silently
no-op) when the capability is false. PR 3's, PR 19's, PR 21's, and PR 22's own original test suites
(`test_workspace_export_import.py`, `test_vw_capabilities.py`, `test_vw_workspace_indexeddb.py`,
`test_vw_workspace_schema_version.py`, `test_shared_workspace.py`, `test_f_workspace_reopen.py`,
`test_vw_locks.py`, `test_a2_popout.py`) were all re-run by hand against this new code and stayed
fully green with no assertion needing to change — this PR only adds new members/functions and never
touches an existing record/export/UI shape any of them check byte-for-byte (real, actually-counted
numbers: PR 3 `.py` wrapper 2/2, node suite 53/53; PR 19 `.py` wrapper 17/17, node suite 65/65; PR 21
`.py` wrapper 27/27, node suite 43/43; PR 22 `.py` wrapper 15/15, node suite 47/47; PR 2 (`test_shared_workspace.py`)
`.py` wrapper 2/2, node suite 73/73; PR 16 (`test_f_workspace_reopen.py`, single flat count) 40/40;
PR 20 `.py` wrapper 18/18, node suite 37/37; PR 14 (`test_a2_popout.py`, no separate node file — structural
only, by its own docstring) 62/62). **Proven load-bearing by breaking 9
representative guarantees one at a time** in the working tree and confirming a genuine failure (or,
in several cases, a hard crash proving the guard's absence is actively dangerous, not just
untested) every time, then a clean re-run on revert: no handle remembered after a fresh native
export (5 failures); permission never re-checked before reuse (5 failures); the AbortError
special-case removed from the export path (chain-aborting failure) and from the import path (same);
the fallback branch disabled entirely (crash — `showSaveFilePicker` called on a tier that doesn't
have it); the native branch's JSON payload corrupted (crash downstream, plus a direct mismatch
failure); `importFileNative()`'s shared-validation call replaced with a naive, unvalidated
`workspaceCreate()` (the schema-refusal test no longer rejects); the imported handle no longer
remembered (crash — the next export finds no handle and calls the picker with no mock resolution
value); and the capability-false guard inverted (crash — `showOpenFilePicker` called on a tier that
doesn't have it, proving "rejects clearly" is not merely a style choice but the only thing standing
between a normal call and a synchronous `TypeError`). `rps_lint.py` clean.

**Manual-only, per the plan's own explicit requirement:** the real native Save/Open dialog behavior
on real Chromium hardware — the actual file picker appearing, a technician picking a real file,
write-back-in-place genuinely landing on a real shared/network file across two separate saves — has
no headless test-suite equivalent and is not automated here; the plan's own PR 23 entry states this
plainly ("manual PR note for the real native-dialog path, since a file picker cannot be driven
headlessly in this test suite"). Everything automatable (both capability branches, permission
re-verification, cancel handling, the shared-validation routing, and the handle-remembering
lifecycle) is covered by real, load-bearing, executed assertions above.

## [1.76.0] — 2026-09-05 — `VW.workspace`: schema-versioned saved data (multi-window support, PR 22/25)

Stage 6, PR 22 of `docs/superpowers/specs/2026-09-03-multi-window-tabs-plan.md` — "Adds `schemaVersion`
to the stored record shape; a migration-or-clean-refusal path for a version the running code doesn't
recognize." Depends on PR 2 (CRUD, already merged) **only** — explicitly independent of PR 19–21's
IndexedDB work: this applies equally regardless of which backing (`localStorage` or IndexedDB) is
currently active, since both ultimately store the same record shape. Fulfills the `schemaVersion`
deferral PR 3 named explicitly at the time it shipped ("deliberately out of scope... `schemaVersion`
and migration-on-read (Stage 6)").

**What changed in the record shape:** `{ id, name, items, created, lastOpened, source }` gains a
seventh field, `schemaVersion`, per the design spec's own record-shape block. A new
`_WS_SCHEMA_VERSION = 1` constant is stamped onto every NEWLY-created record by `workspaceCreate()`.

**The three cases, and the ONE shared decision function both call sites use:** `_wsClassifyRecordSchema`
is the single place the `<=`/`>` comparison against `_WS_SCHEMA_VERSION` is made — both the read-path
migration (`_wsMigrateAll`, below) and the import-path validation (`_wsValidateImportShape`, PR 3) call
this exact function, never a second, independently-typed copy of it.
- **MISSING entirely** — every record PR 2-through-21 ever saved, the overwhelmingly common real case,
  not a hypothetical one: a trivially safe migration, since the record shape itself hasn't otherwise
  changed. Stamped with the current constant in place.
- **Present and `<= _WS_SCHEMA_VERSION`** — understood, passed through untouched. Only one version has
  ever existed as of this PR, so this is a straight pass-through today; a real future version bump's own
  forward-migration would branch on the specific old number inside the one function that already
  inspects every record.
- **Present and `> _WS_SCHEMA_VERSION`** — data written by a NEWER build than what is currently running
  (a technician's browser cache holding a newer build, or a rolled-back deploy running older code
  against already-upgraded data — both real, not hypothetical). **Clean refusal:** never silently
  treated as understood, and — just as importantly — never deleted either. A rolled-back/stale build
  committing an unrelated write must not destroy data a newer build already wrote just because this
  build cannot interpret it.

**Where refusal is decided operationally, and why:** for `list()`, the refused record is excluded from
the returned array while every OTHER valid record still comes back correctly — matching `_wsItems()`'s
own established "drop what's invalid, keep the rest" precedent, applied here to *reads* rather than to
the record's continued existence in storage (it stays in the underlying array, untouched, so a
subsequent commit for an unrelated reason cannot silently erase it). For `get(id)`, returning `null`
matches the function's existing not-found convention, but this must be — and is — distinguishable from a
genuine not-found: a `console.warn` names the id and its unrecognized version every time `_wsMigrateAll`
processes such a record, and a new debug accessor, `VW.workspace._lastGetSchemaRefusal()` (mirrored by
`_lastReadSchemaRefusals()` for `list()`), lets a caller or a future UI tell the two apart — deliberately
NOT part of the documented public `VW.workspace` shape, the same convention PR 20's `_debugPendingCount`
established.

**Migration write-back, and TWO real correctness hazards this PR's own load-bearing verification found
and fixed before shipping (not shipped blind):** a migrated (missing-schemaVersion) record is written
back via the existing `_wsCommit()` path so it never needs re-migrating on every future read — an
idempotent, one-time upgrade-on-first-touch, applied at the ONE shared chokepoint PR 21 itself
established (`_wsAllForRead()`/`_wsAllForMutation()`), never duplicated per-consumer.
1. An eager write-back on EVERY migrated read would have silently defeated `_wsRead()`'s own
   pre-existing, deliberate "a read never rewrites corrupt/hostile storage" guarantee (PR 2's own test)
   whenever a migratable valid record shared storage with genuine junk entries — the exact scenario PR
   2's `test_workspace_node.js` already covered (`"array with junk entries mixed in"`). Fixed with a new
   `_wsLastReadHadJunk` signal, set by `_wsRead()` on every call, that defers the durable write
   specifically in that mixed case: the stamp still applies in-memory for that call's own return value
   (nothing is ever answered incorrectly); only the persist waits for a real mutation (which already
   commits unconditionally) or a later clean read.
2. An eager write-back on the IndexedDB-backed path, fired before PR 21's own bootstrap/reconcile had
   resolved, could open a SECOND, out-of-band IndexedDB connection and write the still-provisional
   localStorage-sourced bootstrap cache into the durable store before reconcile's own read of it
   resolved — making reconcile wrongly conclude "IndexedDB already has records" and skip replacing the
   cache with genuinely-authoritative prior-session data, a real, observed failure during this PR's own
   test-writing (caught by PR 21's own re-run, not by a new test). Fixed by gating the eager commit
   behind `_wsIdbDb` already being an established connection — PR 21's reconcile runs at most once per
   page, so once that connection exists, no bootstrap decision is still pending and the commit is safe.

**Export/import (PR 3):** `_wsExportPayload()` now also carries `schemaVersion` — two technicians
running different app versions handing a workspace file to each other is the single most realistic
real-world scenario for a genuine version mismatch. `_wsValidateImportShape()` calls the SAME
`_wsClassifyRecordSchema()` before `workspaceCreate()` is ever reached, so importing a file with an
unrecognized-future `schemaVersion` is refused with the identical specific-`Error` convention PR 3's
other malformed-import cases already use (`"Workspace import failed: ..."`), and nothing is written to
storage. An old export (no `schemaVersion` field at all — exactly what PR 3 always produced) still
imports cleanly, since "missing" classifies as `ok` on the import path exactly as it does on the read
path — never silently downgraded, never refused for a reason that isn't real.

**Placement:** per the by-now well-established `test_a2_popout.py` cross-PR coupling hazard, all new
code lands after PR 21's IndexedDB section and well before `popoutControl()`'s own section — confirmed
by a source-offset assertion in the new test file.

**New `engine/tests/test_vw_workspace_schema_version.py` + `engine/tests/js/test_vw_workspace_schema_version_node.js`**
— the `.py` wrapper runs 13 static source-level/placement/ES5 checks (including the "not duplicated"
structural guarantee: `_wsClassifyRecordSchema` declared exactly once, called from within both
`_wsMigrateAll` and `_wsValidateImportShape`, with a negative check ruling out a second hand-rolled
`schemaVersion >` comparison anywhere in the import-validation function) plus a `node --check` syntax
gate and the node suite's own rollup (15 items, 15/0); that node suite itself carries **47 real
assertions** against the actual shipped `shared.js`, on the localStorage backing deliberately (per this
PR's own IndexedDB-independence): a fresh `create()` stamping the constant proven IN ISOLATION —
checked against raw storage before any `get()`/`list()` call, since a subsequent read would itself
migrate-and-mask a `create()` that forgot to stamp one at all; a hand-constructed old-shaped fixture (no
`schemaVersion` field, exactly what every real PR2-through-21 saved workspace looks like today) read
correctly via `list()`/`get()` AND durably re-stamped in the backing store afterward, proven by reading
raw storage back out, not by trusting the API's own return value; a future-schemaVersion fixture
excluded from `list()` while a sibling record still comes back correctly, proven NEVER deleted or
mutated in storage even across a REAL, unrelated `create()` commit made afterward (checked
byte-for-byte before and after); `get()`'s `null` on that same id proven distinguishable from a genuine
not-found via the new debug accessor; an invalid (non-numeric) `schemaVersion` refused the same way;
export carrying `schemaVersion`, an old export still importing cleanly, and a future-schemaVersion
import throwing/rejecting with a version-naming message while writing nothing to storage (both
...
constant — 2 failures, caught only once the create-time check was isolated from a masking
migrate-on-read (the first version of this test called `get()` right after `create()`, which would
itself have silently fixed the omission); (2) the missing-schemaVersion stamp/write-back removed — 3
failures; (3) the future-schemaVersion refusal disabled — 6 failures; (4) a future record surviving only
reads but not a real mutation commit — 2 failures, caught only after strengthening the test to exercise
an actual `create()` commit (the original read-only check was accidentally vacuous, since nothing in
that scenario ever needed writing back in the first place); (5) export dropping `schemaVersion` — 2
behavioral + 1 structural failure; (6) import skipping the shared classifier — 4 behavioral + 1
structural failure; (7) a hand-rolled second `schemaVersion >` comparison added alongside the shared
call in the import path — 2 behavioral + 1 structural failure, exactly the "not duplicated" guarantee.
`rps_lint.py` clean throughout (the same backtick/ellipsis/`"async "` false-positive class earlier PRs
this session already hit — markdown-style backticks around variable names in new prose comments, a
literal `"..."` inside quoted spec text, and `"async write-through"` — caught and reworded rather than
suppressed).

## [1.75.0] — 2026-09-05 — `VW.workspace`: IndexedDB storage migration (multi-window support, PR 21/25)

Stage 6, PR 21 of `docs/superpowers/specs/2026-09-03-multi-window-tabs-plan.md` — "Swaps the storage
backing PR 2 built (`localStorage` under `viewer_workspaces`) for IndexedDB, keeping
`create/list/get/touch`'s public contract byte-for-byte identical — nothing above this layer changes."
Depends on PR 2 (CRUD, already merged) and PR 19 (`VW.capabilities.indexedDB`, `[1.73.0]`) — `lite`/
`legacy` tier keeps the original `localStorage` path unchanged, exactly as the plan requires.

**The unavoidable problem, stated plainly:** IndexedDB has no synchronous read or write anywhere —
every operation is callback/event-based. `create/list/get/touch/delete` are called synchronously by
every existing consumer today (`workspaces.html`'s list render, `jobcard.html`/`solve.html`'s launch
buttons, `palette.js`), and per the plan's own text none of them may be made to start handling a
Promise or a callback in this PR. A function cannot read or write IndexedDB and return a real value on
the same synchronous call, so this cannot be "the same functions, but reading IndexedDB now" — it has
to be a synchronous cache with IndexedDB underneath it.

**The resolution — a synchronous in-memory cache (`_wsCache`), bootstrapped instantly and losslessly
from a plain `_wsRead()` (localStorage is already synchronous), with IndexedDB reconciled in
afterward, fully asynchronously:**
1. `_wsEnsureCache()` runs on the first workspace call a page ever makes. If the cache is still `null`
   it is set to `_wsRead()`'s result right there, synchronously, before anything else — so even the
   very first call a page ever makes sees a result that really did come from a valid read.
2. Immediately after, `_wsIdbReconcile()` fires fire-and-forget: if IndexedDB already holds records (a
   prior session's own migration), it becomes authoritative and the cache is replaced wholesale; if
   IndexedDB reads back empty (first time on this profile), a **one-time migration** writes the
   localStorage-sourced cache into it. localStorage's own key is deliberately left untouched afterward
   (never cleared) — a frozen, unread backup costs nothing to keep and turns a hypothetical rollback of
   this PR into "reads the last-known-good copy again" instead of a full data loss.
3. From then on every mutation updates the cache synchronously (the calling tab always sees an
   instantly-consistent result, matching today's behavior exactly) and fires an async, best-effort
   write-through to IndexedDB. Once a page is on this path it never writes to localStorage again —
   which is exactly what lets a payload too big for localStorage's own quota succeed.
4. Any IndexedDB failure (blocked/denied open, a thrown exception, a failed transaction) degrades
   silently at the persistence layer only — the cache stays authoritative regardless. **Deliberate
   exception:** a *repeated* failure streak (3 in a row) fires one one-time toast, on this file's own
   R13 "fail loud enough to be seen, never silently misrepresent" discipline — past that point the
   tab's workspace data genuinely only lives in memory, a real and otherwise-invisible data-loss risk,
   not merely a transient hiccup the cache already absorbs.

**Which backing a page uses is decided once, at bootstrap, and latched for the page's life** — the one
deliberate departure from "gate on the live capabilities getter" being a per-call re-read that
`VW.locks` uses (switching lock implementations mid-call is harmless; switching workspace storage
mid-session is not, since `VW.capabilities`'s own fields are live reads that can flip once `RPS.mode`
resolves from its `"modern"` default to the real tier after this file has already loaded). Re-deriving
the backing on every call would risk silently stranding an already-committed cache entry against a
resumed `localStorage`-only path that never saw it.

**The one real limitation this design cannot fully eliminate, stated plainly rather than glossed over**
(the same honesty this initiative has applied to every real-hardware/real-timing edge case since PR 5):
open the app in two tabs at once on modern tier, and each bootstraps its own cache from its own
synchronous localStorage read at whatever moment it first needs one, then reconciles against IndexedDB
independently and asynchronously. There is a real, narrow window where the two tabs' caches can briefly
disagree before both round trips finish, and this PR adds no new cross-tab live-sync mechanism for
workspace data to close it. PR 2's existing `VW.channel` broadcast (`_wsNotify`) is unchanged and still
fires on every mutation, but a receiving tab's own reconcile may not have settled yet when that
notification arrives. Deliberately **not** layering a second broadcast onto the async cache-replacement
step to paper over this: a receiving tab has no way to tell "my own reconcile already finished" from
"it has not", so acting on such a broadcast could paint an intermediate, still-bootstrapping view with
false confidence as easily as a correct one — worse than the narrow, self-healing window this already
is (every later call on that tab, once its own reconcile lands, is correct again).

**Where this lives:** `create/list/get/touch/delete` were modified IN PLACE (never duplicated) to route
through two new entry points, `_wsAllForRead()`/`_wsAllForMutation()`, and commit through `_wsCommit()`
— on `lite`/`legacy` tier these are exactly `_wsRead()`/`_wsWrite()`, unchanged. PR 3's
`exportUrl`/`exportFile`/`importUrl`/`importFile` needed **zero** additional changes — they call
`workspaceGet`/`workspaceCreate` internally and inherit the correct backing for free. Shape validation
(`_wsCoerceAll`, pulled out of the old `_wsRead()`) is shared by both backings so "what counts as a
valid stored workspace" cannot drift between them. Per the by-now well-established
`test_a2_popout.py` cross-PR coupling hazard, all new code lands well before `popoutControl()`'s own
section — confirmed by a source-offset assertion in the new test file.

**New `engine/tests/test_vw_workspace_indexeddb.py` + `engine/tests/js/test_vw_workspace_indexeddb_node.js`**
— the `.py` wrapper runs 24 static source-level checks directly against `shared.js`'s text plus a
`node --check` syntax gate and the node behavioral suite's own pass/fail rollup (26 items, 26/0); that
node suite itself carries **43 real assertions** against the actual shipped `shared.js` loaded into a
`vm.createContext` sandbox, in the same style PR 20
established, with a hand-rolled mock IndexedDB (genuinely asynchronous via `setTimeout`, driven only
through the real `onsuccess`/`onerror`/`onupgradeneeded`/`oncomplete`/`onabort` callback properties
`shared.js` itself assigns): the bootstrap guarantee (seeded localStorage + a **deliberately deferred**
mock `open()` — the very first call is already correct before the deferred open could possibly have
resolved); the one-time migration (proven by reading the mock's own durable store afterward, and that
localStorage's own key survives untouched); wholesale cache replacement once IndexedDB already holds
different records (the first call still serves localStorage's data; a later call reflects IndexedDB's);
mutations that are both instantly synchronous AND eventually durably persisted (create/touch/delete, all
three); `lite`/`legacy` tier (and a raw-API-present-but-`"premium"`-tier case) never calling
`indexedDB.open()` at all, proven via the mock's own call counter; failure resilience (a synchronously-
throwing `open()`, and every operation erroring, never break a synchronous caller); the one-time
repeated-failure toast (a real invocation counter, not just text comparison, proving it fires exactly
once once the streak crosses threshold and never again); and the large-payload case this whole migration
exists for — an ~8MB workspace payload succeeds via IndexedDB (both synchronously and, eventually,
durably) while the identical payload against a quota-constrained (~5MB) localStorage-only mock fails
...
**Proven load-bearing by breaking 7 representative guarantees one at a time** in the working tree and
confirming a clean re-run on revert every time: (1) the synchronous bootstrap returning `[]` instead of
`_wsRead()`'s result — failed 4 tests (bootstrap + migration + cache-replacement); (2) skipping the
one-time migration write — failed exactly the migration-write assertion; (3) skipping the cache-
replacement assignment — failed exactly the 2 cache-replacement assertions; (4) the tier gate reading
raw `indexedDB` presence only, ignoring tier — failed exactly the 3 tier-gate assertions; (5) `_wsCommit`
still writing to localStorage on the IndexedDB path — failed exactly the large-payload and
no-localStorage-write assertions, the entire point of this migration; (6) removing `_wsIdbOpenDb`'s
`try`/`catch` around a synchronously-throwing `open()` — failed exactly the 3 failure-resilience
assertions (the exception propagated into the caller instead of degrading silently); (7) removing the
one-time-toast latch (re-firing on every failure past threshold) — failed exactly the "fires exactly
once" assertion, caught only after upgrading that assertion from a text-equality check to a real
invocation counter (an identical message string on every fire cannot otherwise distinguish one firing
from several). `rps_lint.py` clean throughout.

## [1.74.0] — 2026-09-05 — `VW.locks`: Web Locks API wrapper (multi-window support, PR 20/25)

Stage 6, PR 20 of `docs/superpowers/specs/2026-09-03-multi-window-tabs-plan.md` — "`VW.locks.withLock(name,
fn)`; falls back to a best-effort in-memory single-tab lock on `lite`/`legacy` tier or where
`navigator.locks` is absent." Depends on PR 19 (`VW.capabilities.webLocks`, `[1.73.0]`) — this is the
**first real consumer of `VW.capabilities` anywhere in this codebase**. `withLock(name, fn)` gates on
the live `_capabilities.webLocks` getter directly, exactly the way `VW.capabilities.windowPlacement`
itself calls straight into `_screenPlacementAvailable()` rather than re-deriving its own copy — never a
second, independently-typed `"locks" in navigator && tier === "modern"` check of its own. Writing that
second check would have been exactly the kind of duplicated-logic-that-can-drift PR 19 was built to
rule out.

**`withLock(name, fn)` always returns a Promise settling with whatever `fn()` itself settles with** —
`fn` may return a plain value or a Promise, the same duck-typing `navigator.locks.request()`'s own
callback return value already supports.

**Real-API path** (`_capabilities.webLocks` true): delegates straight to `navigator.locks.request(name,
function (lock) { return fn(); })` — the browser's own implementation already handles lock acquisition,
release-on-settle (success or failure), and cross-tab serialization; nothing here reimplements any of
that.

**Fallback path** (`webLocks` false — `lite`/`legacy` tier, or the raw API absent, or a non-modern tier
even with the raw API technically present): a best-effort in-memory **promise-chain mutex**, scoped to
just this tab — not a real cross-tab guarantee, but "never blocks" (the design doc's own edge case:
correctness within one tab is unaffected; cross-tab races the real API would have prevented become
possible again, an accepted, explicitly-known regression on older hardware, never a silent one). An
internal `_lockQueues` map keys a lock name to its current "tail" promise: a call with a new name chains
off `Promise.resolve()` (runs on the next microtask, "never blocks"); a call sharing an existing name
chains its own `fn()` invocation onto that name's tail, so same-name calls run strictly one at a time,
in call order — genuine same-tab mutual exclusion; different names never share a tail, so they never
wait on each other. The critical design point is that the map's stored tail is always a
**settled-regardless-of-outcome** derivative of each call's own result (`.then(onFulfilled, onRejected)`
with both handlers returning normally) — so a `fn()` that rejects or throws still lets the next queued
call for that name run once it settles; a failure can never permanently jam a name's queue. The promise
actually handed back to the caller, by contrast, is built directly off `fn()`'s real result, so the
original rejection still reaches whoever made that specific call — only the internal chain-advancing
copy swallows it, never the value the caller sees. Once a name's queue drains (no newer call replaced
the tail in the meantime), its map entry is deleted, so `_lockQueues` never grows without bound across a
long session — a debug-only `_debugPendingCount()` introspection hook (deliberately NOT part of the
documented `VW.locks` API surface, which names exactly one member, `withLock`) makes this provable with
a real executed assertion rather than a source-text guess.

**Placement discipline** — per PR 6/PR 17/PR 19's own documented `test_a2_popout.py` cross-PR coupling
hazard (that test slices `popoutControl()`'s body up to the next `var VW = {` marker; anything inserted
between them gets silently swallowed into what it inspects), the new code lands entirely BEFORE
`popoutControl()`'s own section — directly after `VW.capabilities`, the same place PR 19 landed —
confirmed by a source-offset assertion in the new test file and a clean, unchanged re-run of
`test_a2_popout.py` itself (62/0).

**New `engine/tests/test_vw_locks.py` + `engine/tests/js/test_vw_locks_node.js`, 55 real assertions
total** (18 top-level + 37 in the node behavioral layer), against the actual shipped `shared.js`, in the
same `vm.createContext` sandbox style PR 19 established: the real-API path genuinely delegates to
`navigator.locks.request()` with the exact lock name, and a resolved/rejected value — both a plain
value and a Promise — round-trips correctly in both directions; the raw API absent case runs the
fallback, proven by the absence of the synchronous throw a real `navigator.locks.request()` attempt
against `undefined` would cause; **the single most important test** — raw `navigator.locks` genuinely
present, wired to a canary, but tier `"lite"`/`"legacy"`/`"premium"` — proves the fallback still runs
and the canary is NEVER invoked in any of the three cases (the live proof the gate is genuinely
capability-driven, not raw-feature-driven, mirroring PR 19's own most-important live-read test; a
contrasting "tier modern → the same canary IS invoked" case proves the canary mechanism itself can
catch a real call); fallback mutual exclusion (two overlapping same-name calls run strictly one at a
time, in call order, proven via a shared log with intentional delays, asserted to read exactly
`"1 start,1 end,2 start,2 end"`, never interleaved); fallback independence (two different-name calls
are proven to genuinely interleave); fallback resilience (a first call whose `fn()` rejects — one
variant via a rejected Promise, a second variant via a synchronous throw — does not prevent a second
queued call for the same name from running afterward, and the first call's own caller still receives
the exact original rejection, never silently swallowed); and the queue-cleanup guarantee (the debug
pending-count reads 0 once a name's queue fully drains, proven for both a single call and a longer
5-call chained sequence, and proven to read back up while calls are still in flight, so the assertion
genuinely exercises the map shrinking rather than a permanently-zero stub).

**Proven load-bearing by breaking 4 representative guarantees one at a time** in the working tree and
confirming a clean 55/0 on revert every time: (1) making the gate a raw-feature-only check (`"locks" in
navigator`, ignoring tier) — failed exactly the 3 "single most important test" assertions plus 2
source-level checks; (2) removing the fallback's chain-onto-the-prior-tail line (each call starting
fresh) — failed exactly the mutual-exclusion assertion (log read `"1 start,2 start,2 end,1 end"`); (3)
making a failure's promise become the queue's own new tail instead of a settled-regardless-of-outcome
derivative — jammed the queue exactly as predicted (the second queued call inherited the first's
rejection instead of running fresh, observed directly); (4) removing the cleanup `delete` line — failed
exactly the two queue-cleanup assertions. `rps_lint.py` clean.

One pre-existing PR 19 test assertion (`shared_js_exports_capabilities_off_vw` in
`test_vw_capabilities.py`) had baked in the assumption that `capabilities: _capabilities` was the LAST
key in the final `VW` assembly, immediately followed by the object literal's closing `}` — an
assumption this PR's own `locks:` key, appended right after it, legitimately breaks. Fixed by widening
that regex's closing delimiter to accept either `}` (nothing appended yet) or `,` (a later key follows),
preserving the actual guarantee it exists for (`capabilities: _capabilities` still appears immediately
after `checkpoint`'s own block, in that position); re-confirmed clean (17/0) afterward.
...
than a dozen prior entries, going back well before `[1.57.0]` first put a number on it, every single
one reporting the same shrug: reproduced on unmodified `origin/main`, not this change's fault, moving
on.** Asked to open a dedicated fix for it (rather than re-document it a 16th time), the actual root
cause turned out to be different from what every prior entry assumed.

**What it is not:** expensive local computation that has grown too large for a fixed timeout — the
`test_ingest_routes.py`/`[1.50.0]` shape of problem this flake was always filed alongside.

**What it actually is:** `/api/ask` (`ask.answer()`) and `/api/search_hybrid` (`hybrid_search()`) both
lazily `import embed`, and `embed.py`'s `SentenceTransformer("all-MiniLM-L6-v2")` call reaches out to
the live Hugging Face Hub on every fresh process — confirmed directly, not inferred: the exact same
call measured **15.97 seconds** with a real `"unauthenticated requests to the HF Hub"` warning on the
wire, immediately followed by a **consistent 8.4-8.7 seconds across 3 runs** once forced fully offline
(below) using the same already-warm local model cache. That network round trip is unbounded by
design — its cost depends on Hugging Face's own availability and this project's un-authenticated rate
limit at the moment the test happens to run, not on anything this codebase controls — which is exactly
why widening the timeout number never once fixed it in over a dozen attempts: **45s wasn't enough
either**, reproduced live while diagnosing this.

This also directly contradicts a contract this project already claims for itself: `.github/workflows/
ci.yml`'s own header comment states "every one of those test files is self-contained ... no network
egress." This flake was that contract silently unenforced for one specific pair of routes, not a
genuinely slow feature.

**Fix, in `engine/tests/test_routes.py`:** `os.environ.setdefault("HF_HUB_OFFLINE", "1")` and
`os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")`, set once near the top of the file (`setdefault`,
not an overwrite, so an explicit ambient override a developer already set still wins). Verified safe
in both directions a real CI/dev split can produce:
- **A warm local model cache** (any dev machine that has run this before): the real semantic path
  still runs, now purely from disk — 8.4-8.7s, no network variance, confirmed over 3 runs.
- **No cached model at all** (this project's own CI has no Hugging-Face-model caching step, so every
  fresh runner starts from zero): confirmed directly by pointing `HF_HOME` at an empty directory —
  offline mode fails **immediately** (~5s) rather than hanging or retrying, and lands in
  `ask.answer()`'s own pre-existing `except Exception: pass` around `embed.search()`, degrading
  gracefully to FTS-only passages, exactly the fallback path that already exists for any other
  `embed.py` failure. Never a 5xx, never a hang, in either case.
- Kept a modest, no-longer-network-dependent safety margin on top: `SLOW_ROUTE_TIMEOUT` widens just
  these two routes' client timeout from the shared 10s default to 25s (~3x the observed 8.7s warm-
  cache worst case, for a slower CI machine) — every other route keeps the tight 10s default, so a
  genuinely hung or broken route still fails fast. (This replaces an even-less-load-bearing first
  attempt at this same fix that widened it to 45s and still saw a live-network timeout — left in
  version control history, not silently dropped, as the actual proof that a bigger number was never
  the right fix.)

**Trade-off, stated plainly rather than glossed over:** in an environment with no cached model (this
project's CI, today), `/api/ask`'s and `/api/search_hybrid`'s real semantic-retrieval code path is no
longer exercised there at all — only the FTS-fallback path is, same as it always silently was on any
run unlucky enough to have the live download stall past its timeout, just deterministic now instead
of a coin flip. A follow-up that adds a Hugging-Face-model cache step to `.github/workflows/ci.yml`
(mirroring the pip cache already there) would restore full semantic-path coverage in CI without
reintroducing this flake; not done here to keep this fix to the one thing it was scoped for.

**Verified:** `test_routes.py` standalone, 3 consecutive runs: **298 passed, 0 failed** every time (was
297/1, flaking specifically on `/api/ask`, before this fix). Full `engine/tests/verify_all.py
--snapshot`: **75 checks, 75 ok, 0 FAILED — ALL GREEN** (`test_ingest_routes.py`'s own separate,
unrelated real-subprocess-timing flake — confirmed via its own standalone rerun and via `grep` showing
zero `embed`/`import embed` references in that file — also happened to be clean on this run, not
touched by this change).

- **`engine/tests/test_routes.py`**: `os.environ.setdefault("HF_HUB_OFFLINE", "1")` /
  `os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")` added near the top of the file; `_get()` gained
  an optional `timeout=` parameter (default unchanged at 10); new `SLOW_ROUTE_TIMEOUT` dict widens
  `/api/ask` and `/api/search_hybrid` specifically to 25s.
...
Stage 5, PR 17 of `docs/superpowers/specs/2026-09-03-multi-window-tabs-plan.md`, depending on PR 6
(`VW.windows` layout capture/restore, merged as `[1.67.0]`). Extends `VW.windows.open(url, opts)`
with an opt-in `opts.screen` hint: a truthy value means "prefer placing this window on a screen
different from this tab's own, if one exists and is available" — feature-detected via the Window
Management API's `getScreenDetails()`, gated to the hardware tier the design doc's own item 10 asks
for ("modern tier only").

**The doc/code gap, resolved the same way PR 15 resolved an identical one.** The design doc names
`VW.capabilities.windowPlacement` as the feature-detection gate, but `VW.capabilities` is Stage 6
(PR 19–25) and does not exist yet — nothing here builds any part of it. PR 15 hit the identical shape
of gap for `VW.capabilities.tier` and had genuinely nothing real to fall back to, so it shipped
feature-detected but INERT (reads as "no tier info, do nothing" until Stage 6 lands). This PR is not
in that position: `window.RPS.mode` (set by `rps.js` on every page that loads it — `"modern"`/
`"lite"`/`"legacy"`) is a real, already-live hardware-tier signal, and it IS the capability ladder
item 10 asks for, just not yet wrapped in the Stage-6 name. So this gates for real, right now, on
`window.RPS.mode === "modern"` — an exact string match, never a truthy read, since `"premium"` is an
additive visual-effects FLAG layered on top of an already-`"modern"` mode (`rps.js`'s own comment;
confirmed against `rps.py`'s `VALID_MODES`), never a mode value of its own. `window.RPS` is
genuinely `undefined` on the 32 of this app's 49 pages that never load `rps.js` — treated exactly
like "not modern tier," never a throw. A future Stage 6 PR may swap this direct check for
`VW.capabilities.windowPlacement` once that exists, the same way PR 15's own tier-check comment
already points a future PR at itself.
...
prompt, period — `getScreenDetails` is never even referenced.
...
**Every failure mode is caught and silently ignored**, exactly like every other "can't place it,
leave it where the browser already put it" case in this file: the API absent, the permission denied
(a rejected promise), `getScreenDetails()` itself throwing synchronously (e.g. called outside an
active user gesture), only one screen existing, the window having been closed before the promise
resolved. Nothing here can produce an unhandled rejection or a console error under normal denial.
...
extending PR 5's own dual-sandbox convention (the same one PR 6 itself extended): `opts.screen`
absent (or explicitly falsy) NEVER calls
`getScreenDetails()` at all — the single most important guarantee given this feature's stated
...
in the process (a global `process.on("unhandledRejection", …)` listener backs this, not just an
...
returns, with no wait, so a hypothetical await-before-open regression would be caught even before any
promise had a chance to resolve. **Proven load-bearing by breaking 6 representative guarantees one at
a time** in the working tree (dropping the opts.screen truthiness guard; widening the tier gate to
admit `"lite"`; removing the pick function's identity/key check, twice — once against the 2-screen
case and once against the 1-screen length guard; removing the `["catch"]` handler; removing the
feature-detection check together with the outer `try`/`catch`; and inserting a premature
`_attemptScreenPlacement()` call before `window.open()`) and confirming the right assertions
genuinely failed each time (3, 2, 2, 3, 1, 4 respectively), then reverting and re-confirming a clean
32/0. `rps_lint.py` clean (`shared.js` is ES5-required; two prose word choices — "let alone" and
"never let" — read as false-positive ES6 `let` hits and were reworded, same near-miss category PR
6's own entry already named).

**Full `verify_all.py` run specifically to catch PR 6's own named `test_a2_popout.py` cross-PR
test-coupling hazard** (it slices `popoutControl()`'s body up to the next `"var VW = {"` marker, so
anything inserted between them gets swallowed into what it inspects) — avoided by construction, same
as PR 6: every new function lands immediately after `windowsRestoreLayout()`, before the bench/
checkpoint/`popoutControl()` sections, never between `popoutControl()` and the final `VW` assembly.
Confirmed `test_a2_popout.py` unaffected at a clean 62/0.
...
used for every other real-browser-only behavior since PR 5.
...
plan doc's own stage order, same shape as PR 3's own out-of-order landing: it belongs right after PR
5 (open/reuse/toast core) but was skipped over during this session's earlier parallel-dispatch of
other PRs, and is inserted now because PR 17 (C — screen-aware placement, next in the queue)
explicitly depends on it existing first.

**`registry()` now reports LIVE `screenX`/`screenY`/`outerWidth`/`outerHeight`** per tracked window,
read directly off the window handle `_winReg` already holds — at CALL time, not captured once at
open-time and cached, since a technician can move or resize a window after opening it and re-reading
the live handle costs nothing extra. Every property read is guarded independently: a handle that
throws or returns something non-numeric on ONE field degrades ONLY that field to `null`, never the
other three, and never any OTHER tracked window's entry in the same `registry()` call. This app only
ever opens its own pages via `window.open()`, so every handle is same-origin and these are ordinary,
no-permission-needed properties — nothing here needed the feature-detected `getScreenDetails()` API
PR 17 layers on top of this, explicitly out of scope.

**`windowsOpen(url, opts)` now optionally accepts a position/size HINT**: `opts.left`/`opts.top`/
`opts.width`/`opts.height`, matching `window.open()`'s own features-string vocabulary directly (one
translation step, not two). Threaded into the standard THIRD `window.open()` argument
(`"left=..,top=..,width=..,height=.."`) **only when a genuinely NEW window is being opened — never on
a reuse**, even when the reusing call itself offers hints: browsers generally only honor
position/size features on a window's very first open, a real, honest, stated-plainly browser
limitation this PR does not paper over. Every hint is sanity-checked against THIS screen's own
`window.screen.availWidth`/`availHeight` before use (a generous 4x ceiling, wide enough to admit a
plausible second-or-third-monitor position, tight enough to still catch a wildly stale or
just-plain-nonsensical value) — the design doc's own named "monitor unplugged since the position was
saved" fallback case. A hint that fails is dropped ENTIRELY, never partially applied, degrading
silently to the browser's own default placement; a bad hint, or an unreadable/non-positive
`window.screen`, never throws.

**New `VW.windows.restoreLayout(entries)`** — takes an array shaped like what `registry()` returns
(or a saved snapshot of it) and, for each entry carrying a usable `name` AND `url`, calls
`windowsOpen()` — the SAME open/reuse/toast/broadcast path above, not a second, parallel copy of any
of it — translating `screenX`/`screenY`/`outerWidth`/`outerHeight` into `windowsOpen()`'s own
`left`/`top`/`width`/`height` opts. An entry missing a usable name or url is skipped, never thrown
over, and never aborts the rest of the batch. Returns one result object per INPUT entry, in order:
`{name, url, ok, reused}`. **Must never be called from a load/init/`DOMContentLoaded`-style handler
anywhere in this codebase** — restoring a technician's window layout unprompted is exactly the "a web
page cannot run code 'on app launch' unprompted" case the design doc's own honest note names; nothing
in this diff wires one, since the button that will eventually call it is a later PR's job (this PR is
API-only, matching PR 2/PR 3/PR 5's own precedent of shipping without a dedicated UI page).

**New `engine/tests/test_windows_layout.py` + `tests/js/test_windows_layout_node.js`, 51 real
assertions** (41 behavioral, run through the real production code in a `vm.createContext()` sandbox
extending PR 5's own dual-sandbox convention, + 10 static source-level checks) — not source-text
matching for the behavioral half, actual calls through the real exported `VW.windows.registry()`/
`open()`/`restoreLayout()` functions: a real handle's bounds mutated between two `registry()` calls
proving the read is genuinely live; a throwing property on one tracked window proven to degrade only
that field, only that window, leaving a second tracked window's entry untouched in the same call; a
sane hint proven to thread a real features string on a new open and NEVER on a reuse even when the
reusing call offers one; an implausible hint, a missing `window.screen`, a zero `availWidth`, and a
throwing `window.screen` accessor all proven to degrade gracefully rather than throw; `restoreLayout`
proven to call through the real `windowsOpen()` path via the SAME broadcast-channel envelope shape
landing on a genuinely separate listener tab (not a re-implemented parallel path), skipping 3
deliberately malformed entries mixed into a 5-entry batch without aborting the 2 well-formed ones. The
static half proves `restoreLayout` is never invoked anywhere in this diff, two independent ways: a
comment-stripped full-source scan of `shared.js` for any real call-site syntax (zero found, only the
function's own declaration), cross-referenced against a `git diff`-scoped scan of this PR's own added
lines (best-effort, degrades to skipped rather than false-failing without `origin/main` history), plus
a belt-and-suspenders check that no `addEventListener("DOMContentLoaded"/"load"/"pagehide", …)`
handler body anywhere in `shared.js` mentions `restoreLayout` at all, even in a comment. **Proven
load-bearing by breaking 6 representative guarantees one at a time** (the live-not-cached read, the
per-field throw independence, the implausible-hint drop, the reuse-never-threads-bounds rule, the
malformed-entry skip, and the load-handler-never-calls-it check) and confirming the right assertions
genuinely failed each time (5, 1, 5, 2, 7, 1 respectively), then reverting and re-confirming a clean
51/0. **PR 5's own existing test updated, not broken around**: `test_windows_node.js`'s "registry
entry exposes only name/url ... no layout fields yet -- that is PR 6" assertion is now the assertion
that those fields exist (`null` in that test's own mock-handle harness, which sets no bounds of its
own — the real live-bounds behavior is what the new dedicated suite above proves). `rps_lint.py` clean
(`shared.js` is ES5-required; two prose word choices — "let alone" and an "…" ellipsis in a new
comment — read as false-positive ES6 `let`/spread-rest hits and were reworded, not suppressed, same
category of near-miss PR 3's own `[1.65.0]` entry already named). Design doc's own `VW.windows` item-4
header comment updated from "PR 5 — in progress: open/reuse/toast core; layout capture/restore is PR
6" to "PR 5 + PR 6 landed" — nothing else in that spec file touched, matching the PR's own scope note.

**Deliberately out of scope, matching this PR's own plan-doc scope, not a shortfall:** any dedicated
UI page or button calling `restoreLayout()` (PR 17 and/or a later PR is the real consumer — same
API-only precedent PR 2/PR 3/PR 5 already set); the feature-detected, permission-gated
`getScreenDetails()` multi-monitor placement API (PR 17's own job, explicitly named as depending on
this PR); and the actual on-screen placement behavior in a real browser, on real hardware — Node has
no `window.open` to be right or wrong about whether position/size features are genuinely honored on a
new window and genuinely ignored on a reuse, or about whether the "monitor unplugged" fallback
actually lands somewhere a technician can find. Both are stated plainly as manual, real-hardware-only
checks in the PR body, the same honest framing this initiative has used for every other
real-browser-only behavior since PR 5.
...
gave the home nav a ↗ pop-out next to every tool link; A2 is the mirror image — a page a technician
is *already on* now gets its own control to pop itself out into a second window, so popping `/torque`
onto a second monitor no longer requires navigating back to the home nav first.

**`engine/ui/shared.js` — `VW.popoutControl()`.** Called once, zero-config, by a page's own inline
script. Reads `location.pathname`/`location.search`/`document.title` itself and injects a real,
keyboard-focusable `<button id="vw-popout-pill">` (never a `div`+click handler — the same
`[1.46.0]`/`[1.47.0]` accessibility convention every prior pop-out control in this app follows),
labeled `"Open <page> in a new window"` (A1's own phrasing). One shared inner `doPopout()` function
backs both the button's `onclick` and the Ctrl+K palette entry described below — never two
independent copies of the `window.open`/`VW.windows.open` call.

**Window naming matches A1's `popoutName()` (`index.html`, ~line 592) exactly** — `_popoutWindowName()`
is a deliberate byte-for-byte copy of that transform (strip query, strip hash, strip leading/trailing
slashes, replace unsafe-char runs with `-`, prefix `vw-`, fall back to `home`). This is the entire
reason A1's own comment named this PR: a technician who pops `/torque` out from the home nav and then,
already on `/torque`, clicks *this page's own* new control lands on the SAME window, not a second one
stacking up beside it. `engine/tests/test_a2_popout.py` extracts and compares the two files' actual
regex/string-transform source text (not eyeballed) so the two copies can't silently drift apart.

**Ctrl+K palette entry, order-independent.** `popoutControl()` cannot reach into `palette.js`'s
`COMMANDS` array directly — on the normal load order (`shared.js` in `<head>`, then the page's own
inline script, then `palette.js` last, right before `</body>`) `COMMANDS` doesn't exist yet at the
moment it runs. Instead it pushes a plain `{ic,label,hint,act}` descriptor onto a new
`window.__paletteQueue` (created lazily). `palette.js` drains that queue into `COMMANDS` at two
points — right after `COMMANDS` is built, and again as the first statement inside `open()` — so a
descriptor lands correctly whichever of the two real script orders a future page ends up using, not
just the one all 5 adopting pages happen to share today.

**Placement (`engine/ui/base.css`): `#vw-popout-pill`, `right:288px; bottom:12px`**, joining the same
fixed pill row as `palette.js`'s `#cmdk-pill` (`right:12px`) / `#bench-pill` (`right:150px`). The
offset isn't a guess: measured directly in a real browser (desktop + this app's own 960px/720px
responsive breakpoints, and in kiosk mode) — `#bench-pill`'s own rendered left edge lands around
`right:217px`, so `right:288px` leaves a genuine ~70px clear gap on every width tested, since
fixed-position elements anchored from the same edge keep the same relative gap regardless of viewport
width. Does **not** touch the separately-filed, pre-existing `#vw-footer`/`#cmdk-pill`/`#bench-pill`
overlap (flagged earlier this session) — this pill sits clear of all three.

**Adopted on the 5 pages named in the plan:** `part.html`, `procedure.html`, `torque.html`,
`jobcard.html`, `bench.html` — each already carrying its PR 8/[1.58.0] responsive pass and both
`/shared.js` and `/palette.js`. Each calls `VW.popoutControl()` once, guarded, between its
`/shared.js` and `/palette.js` script tags.

**Tests:** new `engine/tests/test_a2_popout.py`, 62 source-text/structural assertions (`node --check`
syntax coverage included, skips cleanly without node) — proven load-bearing by reverting each of 5
representative fixes in turn (the function's existence, the naming-transform identity with A1, both
palette drain call sites, one page's adoption call, the pill's placement offset), confirming the
relevant assertion(s) genuinely fail, then restoring and re-confirming a clean 62/0. `rps_lint.py`
clean (`shared.js`/`palette.js`/`procedure.html`/`torque.html` are ES5-required; `part.html`/
`jobcard.html`/`bench.html` are modern-by-design) — but only after reworking this PR's own doc
comments in `shared.js`, which first tripped the exact `[1.51.0]`-documented false positive
(backticks used as plain code-reference punctuation, read as ES6 template literals by the linter's
blunt text scan) on the initial draft.

**What this cannot cover, same as A1:** whether clicking the button (or its palette entry) in a real
browser actually opens/reuses a window — no in-browser runner exists in this suite;
`test_shared_windows.py`'s node-sandbox layer already covers `VW.windows.open()`'s reuse/toast logic
in isolation. The real end-to-end proof (pop out `/torque`, confirm A1's home-nav ↗ for Torque reuses
...
Found while reviewing `/api/coverage` output during the responsive-verification batches (PRs 8-11):
`coverage.html`'s CAD-renders meter could read `156.3%`. Two fixes, one display-side and one
root-cause, both on this version.

**Display (`engine/ui/coverage.html`):** the three real percentage meters (OCR, CAD renders,
Vectorized figures) built their bars via inline string concatenation and never clamped the width,
while the page's own `pctBar()` helper already did the right thing but was dead code — an
out-of-range ratio laid the inner `<i>` out past its track, contained only by `.bar`'s
`overflow:hidden` and visually indistinguishable from exactly 100%. Routed all three meters through
`pctBar`. Per this project's R13 discipline (fail loud, never silently misrepresent), the bar width
is clamped to 0-100 but the **numeric label is not** — it still reads the true ratio, now paired
with a visible "over 100%" flag and an amber/red treatment.

**Root cause (`engine/coverage.py`, `engine/make_cad.py`):** `cad.pct` compares `rendered_v3`
against `representative_parts`, and both were measured in the wrong units against `make_cad.py`'s
actual render pool. Denominator (~99.7% of the gap, measured live: 20,869 vs the real 32,622):
`representative_parts` only counted `ref_nsn` rows with FLIS dimensional characteristics
(`_THREED_WHERE`), but `make_cad.py`'s `_collect()` — the actual render job — unions that with
every NSN appearing in `parts` against a figure (`fig_no IS NOT NULL`); coverage.py was comparing
renders against half the real population. Numerator (present, small in this dataset): `rendered_v3`
did a raw `_v3.png` suffix count in `cadcache/`, but `cad_render.spin_path()` also writes turntable
sprite sheets named `<nsn>_spinNN_v3.png` into the same directory, and the 'modern'/'lite' RPS tiers
both resolve to v3 style at different frame counts (24 vs 16) — one part can leave up to 3 files
ending `_v3.png`. Fix: `coverage.py` gained `_REP_PARTS_SQL` (mirroring `make_cad.py`'s real
`ref_nsn` UNION `parts.fig_no` query, TRIM/empty-string handling matched) for the denominator, and
`_count_cad_v3()`, which excludes `_spin*_v3.png` sprite sheets from the per-part numerator count.
Both files carry sync comments cross-referencing each other so the WHERE clause / `_collect()`
logic doesn't drift apart again. Verified against the live index: `cad.pct` went from `156.3%` to a
clean `100.0%` (32,622 rendered / 32,622 representative parts). `test_property_fuzz.py` (~18k cases,
exercises `coverage.overview()`) re-run clean; both files byte-compiled.
...
**VERSION → `1.61.0`.** Reserved up front: this branch is one of four sibling batches of the same
verification pass being built in parallel, and the other three claim `1.58.0`/`1.59.0`/`1.60.0`. Same
convention `[1.53.0]`/`[1.52.0]`/`[1.57.0]` already used; if a sibling does not land, this simply
takes the next free number on merge, the way `[1.57.0]` itself was renumbered from its reserved
`1.54.0`. The doc-list numbers below (`MASTER-RECONCILIATION` item 44, `PROJECT-SUMMARY` item 43,
`HANDOFF-NOTE`'s thirty-third pass) are reserved the same way and for the same reason.

Stage 3, PR 11 of `docs/superpowers/specs/2026-09-03-multi-window-tabs-plan.md` — the **last** of the
four per-page batches that turn `[1.57.0]`'s shared CSS from "written" into "verified". `[1.57.0]`
said plainly that not one real page had been opened in a resized window; this batch does that for
twelve of them, in a real browser, against the real server.

**The 12 pages, and why they are one batch.** `master.html`, `mastercov.html`, `packet.html`,
`exploded.html`, `schematics.html`, `threed.html`, `deepzoom.html`, `stepflow.html`, `keywords.html`,
`publog.html`, `audit.html`, `cadtex_test.html` — the specialized-visualization group. Several of
them (`threed`, `deepzoom`, `schematics`) render a WebGL/canvas/SVG stage that sizes itself by
script inside its own clipped viewport, which is exactly why `base.css` excludes `svg`/`canvas` from
its `max-width:100%` clamp. **The internal rendering of those stages was out of scope and was not
touched**; what was checked around them is the page chrome — toolbars, title bars, card grids,
tables, search rows.

**Method, and one thing about it worth stating because it would otherwise silently produce false
findings.** Each page was served by a real `viewer_app.py` on a real port, opened in a real browser,
and measured at 960px and 720px with `getComputedStyle`/`getBoundingClientRect`, using a probe that
reports every element whose box escapes `documentElement.clientWidth` *and* is not inside a
deliberately-scrolling ancestor. The catch: the browser-automation viewport used here switches into
**mobile device emulation below 768px** (Android UA, `maxTouchPoints:5`), which makes
`(pointer:coarse)` match — and `base.css`'s coarse block sets `min-width:44px` on inputs at
specificity `0,2,1`, which outranks a page's own `.search input{min-width:240px}` at `0,1,1`. The
first 720px reading therefore showed `master.html`'s search box collapsed to **55px**, which is a
real touch-tablet behaviour but *not* the popped-out-desktop-window scenario this PR is about. Every
720px measurement below was re-taken with that one media block surgically disabled (rewriting its
`mediaText`, so only the coarse rules are suppressed and nothing else changes), and `cadtex_test.html`
— which has no `<meta name="viewport">` at all, so emulation falls back to a 980px layout viewport
and hides the defect entirely — was measured at **768px** instead, the widest point that is still
inside the ≤960px band and still on the desktop pointer path.

**Three real defects found and fixed, each in the page's own inline `<style>`, none in `base.css`.**

- **`cadtex_test.html` — 210px of horizontal overflow, the worst in the batch.** `.g` asks for three
  *fixed* tracks: `repeat(3,310px)` + `2 x 14px` gap + `body{margin:20px}` on both sides = **998px of
  content**, against any narrower window. Measured before the fix: at a **768px** viewport
  `documentElement.scrollWidth` **978** vs `clientWidth` **768** — the entire third column of test
  cards and their WebGL canvases off-screen; at **960px**, 978 vs 960, 18px out, clipping the
  right-hand card's canvas. No shared rule can reach this and none should: `.g` is not one of the
  class names the `:where(.grid,.grid2,.cards,.tiles,…)` safety net lists, and `min-width:0` cannot
  shrink a fixed `310px` track anyway. Fixed with `@media (max-width:960px){.g{grid-template-columns:
  repeat(auto-fit,310px)}}` — as many *whole* 310px tracks as actually fit (2 at both 960 and 768),
  chosen deliberately over any rule that would resize the cards, because it leaves the `290x220`
  canvases **completely untouched**: `gl3d.js` owns what is inside them. After: 978 → **753 = 753 at
  768px, zero overflow**, and 945 = 945 at 960px. At 1400px still `310px 310px 310px`, canvas still
  `290x220`, zero overflow — byte-identical (R1). The page still self-reports `SHADER OK 6/6`.
- **`deepzoom.html` — 77px of horizontal overflow at 720px.** `.top` is a single flex row of up to
  **11 controls** and declares no `flex-wrap`, and `.top` is not one of the class names `[1.57.0]`'s
  shared wrap rule covers (`header/.bar/.row/.cols/.search/.chips/.toolbar/.tools/.tabs`). That is
  correct of the shared sheet rather than a hole in it, and it was checked rather than assumed:
  `.top` is declared on exactly **two** pages in this app, here and `pmcs.html`, and `pmcs.html`
  already sets `flex-wrap:wrap` itself — a one-page gap. The failure needs `#edbtn` (Editions) and
  `#pqabtn` (Ask this page) visible, which are `display:none` on a bare host but switch on for real
  whenever `index/dedup.db` holds another edition of the manual, or the host is GPU-tier: at 720px,
  `scrollWidth` **797** vs `clientWidth` **720**, with `#cinfo` laid out at `711..797`, entirely past
  the right edge. Fixed with `@media (max-width:960px){.top{flex-wrap:wrap}}` → **797 → 720, zero
  overflow**, bar wraps to 3 rows and every button label fits on one line instead of stacking.
  Scoped to ≤960px for two measured reasons: it is *enough* (at 960px the same all-buttons row still
  fits unwrapped — the controls merely compress), and it keeps R1 exact — at 1400px every child of
  `.top` sits at byte-identical coordinates to before, in **both** the default and the all-buttons
  configuration. `#stage`, the deep-zoom canvas viewport, was not touched.
- **`schematics.html` — the sheet viewer's title collapsing to 3px.** Not an overflow; a legibility
  failure in the chrome. `.gbar` is one wrapping flex row of ~15 fixed controls and `.gbar .sp` (the
  title of the sheet on screen) is `flex:1 1 0%`, so it gets only the leftover space on its own flex
  line, absorbing the rest through `nowrap` + `ellipsis`. With `24 V SUPPLY (JERRV) COUGAR` open,
  needing **182px**: the title measured **66px at 1400px, 60px at 960px, and 3px at 720px** — not one
  legible character, while the operator is still looking at the drawing. A `min-width` floor was
  tried first and rejected: it only steals the space back from the control row. Since the bar has
  already wrapped at these widths anyway, the title takes a row of its own —
  `@media (max-width:960px){.gbar .sp{flex:0 0 100%}}` → **917px at 960px, 692px at 720px**, fully
  readable, at a cost of one extra ~25px bar row (`.gbar` 99 → 124px) that the stage absorbs. At
  1400px: title 66px, `.gbar` 99px, `.gstage` 706px — exactly the pre-change numbers (R1).

**Nine pages needed no change, and that is a measured result, not an assumption.** `master.html`,
`mastercov.html`, `packet.html`, `exploded.html`, `threed.html`, `stepflow.html`, `keywords.html`,
`publog.html`, `audit.html` all measured `scrollWidth == clientWidth` with **zero** escaping
elements at both 960px and 720px. Where a page's data was not built on this host (the Masterfile,
PUBLOG, provenance and figure-parts tables are all empty here) the page's *own* render output was
injected verbatim — the exact markup its `renderFiltered`/`renderRaw`/`renderRecord`/`renderList`
functions build, with realistic long NSNs, CAGE codes and characteristic strings — so the tables and
card lists were actually exercised rather than measured empty. Two specific things fell out of that:

- `stepflow.html`'s `.bar` declares no `flex-wrap` of its own and gets `wrap` **from `base.css`** —
  a case of `[1.57.0]`'s shared rule doing real work on a real page, confirmed by reading the
  computed value.
- `threed.html`'s viewer bar was stress-tested with a worst-case long part title at 720px: it wraps
  within its column and grows the bar to 101px rather than overflowing. Its `.gside` is a fixed
  320px rail, which correctly does **not** match `base.css`'s `body .side{width:100%}` rule.

**`packet.html`, the print-preview page, got the specific check it was owed, and the answer is
better than "probably fine".** The question was whether the new breakpoints leak into print output.
They *do* bind: measured in an iframe at the real printed page box — US Letter 816px and A4 794px,
each minus this page's own `@page{margin:14mm}` (2 x 52.9px) → **710px and 688px** — where
`(max-width:960px)` and `(max-width:720px)` both match. That is harmless, and precisely why: of the
seven rules `[1.57.0]` added, exactly **one** reaches this page, `body{overflow-wrap:break-word}`,
which prevents a long NSN pushing `table.parts` off the paper. The page carries none of
`.grid/.grid2/.cards/.tiles/.cols/.chips/.tabs/.side`, so every other rule is inert; its only `<img>`
is the QR at an inline `width:74px`; and its screen-only `.toolbar` is `display:none!important` in
print regardless, with `#vw-footer` already covered by `base.css`'s own `@media print` rule. **No
screen-only chrome leaks into the printed sheet, and `packet.html` needed no change.**

**One honest negative result.** `publog.html` at 720px was expected to demonstrate
`body{overflow-wrap:break-word}` earning its keep on a long unbreakable identifier. Measured with the
rule and with it forced back to `normal`: `scrollWidth` **720 both ways** — the real characteristic
string (`TRUCK,UTILITY,CARGO/TROOP CARRIER,1-1/4 TON,4X4,M998A1;M1097A2;M1113`) breaks at its own
commas anyway. Recorded rather than quietly dropped, because the tempting version of this sentence
would have claimed a benefit that is not there on this page.

**One pre-existing defect found and deliberately NOT fixed here.** The shared bottom-right chrome
overlaps itself: `#vw-footer` (bottom:52px) bottom-edge 848 against `#bench-pill`/`#cmdk-pill`
top-edge 844 — a 4px collision — and `#vw-read-btn` overlapping `#bench-pill` by 21px. `base.css`'s
own comment says `bottom:52px keeps it clear of palette.js's pills`, which stopped being true once
those pills became 44px tall. It is **width-independent** (measured identically at 1400px with
`pointer:fine`), so it is not a responsive regression, it predates this whole initiative, and it
affects all 46 pages equally — fixing it in a 12-page batch would mean editing shared chrome that
three sibling branches are working alongside. Named here so it is on the record rather than silently
observed.

**`base.css` was not touched, and that is asserted rather than promised.** Three sibling batches of
this same pass are in flight and the shared sheet is the one file they could collide on. Every fix
above is scoped to one page's own `<style>`, and the new test suite re-asserts that all six of
`[1.57.0]`'s rules are still present and intact, including that `svg`/`canvas` remain excluded from
the image clamp — the premise the whole "stages not touched" claim rests on.

**ES5 classification checked at the source, not inferred from the gate's printout.** `rps_lint.py`
prints `[ ok ] … ES5-clean` for an ES5-required page *and* for a modern page that happens to contain
no ES6, so the output alone cannot tell them apart; the authoritative lists are `ES5_REQUIRED` /
`MODERN_BY_DESIGN` in the gate's own source. In this batch that makes **`packet.html`,
`stepflow.html`, `keywords.html`** ES5-required and the other nine modern-by-design. Moot in
practice — **this PR changes no JavaScript on any page, only CSS** — but the new suite now guards the
three ES5-required pages' inline scripts directly, so an accidental ES6 edit trips the batch's own
tests and not only the gate.

**New: `engine/tests/test_responsive_batch4.py` — 58 checks, and it is not vacuous.** Auto-discovered
by `verify_all.py`'s glob. It states its own limits up front: it has no browser, so it does not
re-assert pixel numbers; what it locks down is that the three fixes are still in the three files,
still scoped inside `@media (max-width:960px)` (the R1 guard — a fix escaping its breakpoint is
caught), that the structural preconditions each fix depends on have not drifted, that the canvases
kept their fixed sizing, that `packet.html` still has none of the shared layout classes and still
hides its toolbar in print, and that `base.css` is unmodified. Two checks are real arithmetic over
numbers parsed from the page's own CSS rather than string matching: `cadtex_test.html`'s natural
width is recomputed from its own track/gap/margin values (`3*310 + 2*14 + 2*20 = 998 > 960`) and the
`@media` rule is only *required* while that arithmetic still overflows — so shrinking the cards later
stops demanding a rule that would no longer be needed, instead of failing spuriously. Proven
load-bearing by mutation: deleting each of the three fixes, moving `deepzoom`'s wrap out of its
breakpoint, and giving `cadtex_test`'s canvas a percentage width were each injected in turn and every
one was caught (exit 1, naming the specific check).
...
- `python engine/tests/rps_lint.py` → **`RPS GATE: PASS -- every ES5-required page is ES5-clean
  (10 modern-by-design pages noted)`**.
- `python engine/build_iteration_snapshot.py` → **`R10 integrity OK`**.
- `python engine/tests/verify_all.py --snapshot` → **`67 checks | 67 ok | 0 FAILED` ·
  `ALL GREEN -- suites pass and every protected file matches the vault.`** All **64** `test_*.py`
  suites PASS (63 pre-existing plus this PR's new one), including `test_uiux_fixes.py` at
  **273 passed, 0 failed** — the suite that string-splits `engine/ui/base.css` itself, and therefore
  the independent check that this PR really did leave the shared sheet alone — plus
  `test_routes.py` **296 passed, 0 failed** with no sign of the known pre-existing `/api/ask` timeout
  flake, and `safeguard verify vs SNAP_20260904_093848_pre-ingest: 737 files, 737 OK, 0 DAMAGED`.
**Three full runs, and the one non-green result is written down rather than re-run until it went
away.** `[1.57.0]`'s two documented hazards were checked *before* starting rather than diagnosed
after: free disk (**38.6 GB on `C:`**) and `netstat` on `test_ingest_routes.py`'s hard-coded **port
8894** (free — no sibling worktree was holding it). The two local servers this PR's browser
measurements used were stopped first, so nothing was mutating the tree mid-run.

1. **Run 1** (code + tests, before the doc edits) — **`67 checks | 67 ok | 0 FAILED`, ALL GREEN**,
   including `test_ingest_routes.py` PASS and `test_http.py` PASS.
2. **Run 2** (confirmatory, after the doc edits) — **`67 checks | 66 ok | 1 FAILED`: `test_http.py`.**
   **The honest part: the failing lines were not captured.** `verify_all.py` prints a failing suite's
   full output *precisely so* this is diagnosable, and `test_http.py` prints `FAILURES (n):` with up
   to 25 `FAIL <url> -> …` lines — but this run's output was read through a 12-line `tail`, which
   discarded exactly the part that mattered. That is a mistake in how the run was observed, not a
   finding about the code, and it is recorded as such. What *is* known: this is **not** the
   documented fixed-port hazard — `test_http.py` calls `_free_port()` and binds an ephemeral port, so
   it cannot collide with a sibling the way `test_ingest_routes.py`'s hard-coded 8894 does. It spins
   a real `viewer_app.py` subprocess with a **10-second** startup budget (50 x 0.2s) and probes 452
   URLs with **8-second** per-request timeouts, and `tasklist` showed **three other `python.exe`
   processes** belonging to sibling worktrees running concurrently on this machine. A load-induced
   `transport error` is therefore the likely explanation — but "likely" is the accurate word, and
   with the failing line discarded it stays inference rather than proof.
3. **Immediately after run 2**, standalone: `python engine/tests/test_http.py` → **`452 requests, 252
   routes` · `no 5xx, all /api responses parseable as JSON.`**, exit **0**.
4. **Run 3** (confirmatory, on the finished tree, full output captured to a file this time) —
   **`67 checks | 67 ok | 0 FAILED`, ALL GREEN**, with `test_http.py` **PASS (18.1s)** and
   `safeguard verify vs SNAP_20260904_094803_pre-ingest: 737 files, 737 OK, 0 DAMAGED`.

So `test_http.py` passed in 2 of 2 full runs where its output was actually preserved, and standalone
in between. It is called out here anyway, at length, because a suite that went red once and green
three times is exactly the kind of thing that gets quietly rounded down to "flake" — and because the
reason it cannot be pinned down further is a capture mistake worth not repeating.

**Ordering note**, the same one `[1.57.0]` records: writing a run's result into the repo modifies the
tree that run just verified. The `ITERATION-SNAPSHOTS.md`/`ITERATION-DASHBOARD.html` regeneration was
therefore done before run 3, and only the numbers in this block were written back after it — a final
post-edit confirmation run is reported in the PR body instead.
...
**VERSION → `1.60.0`.** Stage 3, PR 10 of `docs/superpowers/specs/2026-09-03-multi-window-tabs-plan.md`
— one of the four per-page verification batches `[1.57.0]` explicitly deferred to. Three sibling
batches were being built in parallel off the same `main` and claimed `1.58.0`/`1.59.0`/`1.61.0`, so
this branch reserved `1.60.0` up front rather than race for a number — the same convention
`[1.53.0]`/`[1.55.0]`/`[1.56.0]` already used.

**What `[1.57.0]` left owed, and what this closes.** That entry shipped the shared breakpoints and
said so plainly: *"not one real page has been checked against these rules in a resized window yet."*
This is that check for **11 of the 46 pages** — `learn`, `binaudit`, `coverage`, `ingest`, `ops`,
`status`, `verify`, `command`, `collections`, `review`, `demo`. Every one was loaded from the real
server in a real browser and measured at **960 CSS px** (half a 1080p monitor, the spec's own
scenario) and **720 CSS px** (a docked or quarter-width window), with the page's real data loaded —
not read statically and not assumed correct because the shared sheet exists.

**Three real defects found. Eight pages needed nothing, and that is reported as a measurement, not
as a shrug.**

### 1. `binaudit.html` — every NSN in the audit table split in half, mid-identifier

The audit table is `width:100%` inside a `max-width:1000px` `.wrap`, so its NSN column measures
**127px and holds each NSN on one line at 1440px** — but **123px at 960px** and **94px at 720px**,
at which point the browser takes the hyphens inside an NSN as break opportunities and splits every
identifier across two lines (`6115-01-` / `036-6374`). Measured: **1 line per NSN at 1440px, 2 lines
at both 960px and 720px** — a genuine narrow-window regression, not a pre-existing state.

This is the single worst place in the app for that to happen: `binaudit.html`'s stated job is
flagging **look-alike NSNs** — near-identical numbers that must be told apart digit by digit — and
its own warning copy reads "Easy to mix up; confirm the exact NSN per vehicle." An identifier chopped
across a line break is exactly the failure the page exists to prevent.

**It is not `base.css`'s `overflow-wrap:break-word`, and that was checked rather than assumed.** With
that rule suppressed on the NSN column and re-measured, the NSNs still broke onto 2 lines: a hyphen
is an ordinary soft-wrap opportunity, unrelated to `overflow-wrap`. So this is the per-page override
`[1.57.0]` explicitly left to these PRs ("where a specific identifier must stay intact"), not a
shared-rule bug.

**Two rules, not one, because `white-space:nowrap` alone is not safe.** Measured with only the
nowrap, a ~400px window pushed `documentElement.scrollWidth` to **435 against a 400px client** — real
sideways page scroll. Pairing it with `overflow-x:auto` on `#out` (the static container the table is
rendered into) keeps identifiers intact *and* confines any residual width to the table's own
scroller. Re-measured at 400px with both: page `scrollWidth` **400** = `clientWidth` **400**, `#out`
scrolling internally at **419/368**. Both rules sit inside the page's own `@media (max-width:960px)`
block, so nothing above 960px changes.

Verified from the real file after the edit, at 720px: NSN column back to **127px**, **3 of 3 NSNs on
one line**, page `scrollWidth` **720** = `clientWidth` **720**, `#out` computed `overflow-x: auto`.

### 2. `status.html` — a NIIN variant string split mid-NSN, on the queue built to compare NSN strings

The NIIN format-drift review queue's own description is "Same NIIN written as different NSN strings."
Its `variants` column measures **375px at 960px** (zero of the live rows break) and **232px at
720px**, where the same hyphen-breaking splits a variant. Measured character-by-character with a
`Range` at 720px, the live first row read:

```
5305-00-292-4587  ·  5306-00-292-
4587  ·  5605-00-292-4587
```

— the middle variant cut in half, on the one table whose entire purpose is spotting a one-digit
difference between those strings. Scoped to the **720px** step only, because at 960px nothing breaks.

**Why a wrapper `<div>` and not just the nowrap.** `white-space:nowrap` alone is correct for today's
data — the widest group in this corpus has 3 variants (`884` groups: 877 with 2, 7 with 3), the table
stays 678px, the page does not scroll — but it is not durable. Re-measured with a synthetic
5-variant row, nowrap alone pushed `documentElement.scrollWidth` to **1023 against a 720px client**.
`overflow-x` set on the `<table>` element itself does **nothing** (Chrome keeps computing it
`visible` on a table box — measured, not assumed), so the scroller has to be a real block wrapper.
`<table id="niintbl">` is now inside `<div class="tscroll">`, keeping its `aria-live="polite"`.

Verified after the edit at 720px: **40 live rows, 0 broken variant cells**, page `scrollWidth`
**720** = `clientWidth` **720**; and with the 5-variant stress row re-inserted, the table grows to
982px while the wrapper scrolls it internally (**638 client / 982 scroll**) and the page stays at
720. Proven inert above the breakpoint: at 960px the column widths are **80/375/141/267** with a
863px table — byte-identical to the same measurement taken *before* the change — and
`white-space` computes `normal`; at 1440px, **91/425/160/302** with a 978px table.

### 3. `demo.html` — the guided tour's tooltip placed *behind* the control bar in a narrow window

`place()` clamped the tour tooltip against a hard-coded `barH = 56` — the height this page's fixed
bottom control bar has **only while it fits on one row**. Measured at 720 CSS px the bar is
**119px**. Two independent causes stack, and neither existed when the literal was written: the dots
strip has always carried its own `flex-wrap` and 19 dots stop fitting on one row well before 720px
(**86px** on its own, measured by forcing `flex-wrap:nowrap`), and `[1.57.0]` then added
`flex-wrap:wrap` to the shared `.bar` selector at ≤960px, taking it from **86px to 119px**.

Consequence, measured by stepping the entire 19-step Mechanic tour at **720x620**: steps **3, 14 and
15** placed the tooltip **44px, 3px and 59px behind the control bar** — the bottom of the tooltip
text hidden under it. This is precisely the window shape the multi-window feature makes ordinary.

Fixed by reading the bar's real `offsetHeight` instead of a literal, which is correct at every width
and also repairs the pre-existing 86px case. **ES5 only** — `demo.html` is an RPS/ES5-required page.
After the fix, at 720x620 all 18 measured steps clear the bar (steps 3/14/15 go from **+44/+3/+59**
to **−174/−383/−139**; the worst case across the whole tour is **−5px**, which is the clamp's own
6px margin). Proven inert at desktop width: at **1440x900** the measured bar height is **exactly
56** — the same number the literal hard-coded — with a worst overlap of **−224**; at 960x620 the bar
is 56px and the worst overlap **−13**.

**The `rps_lint` false positive this initiative keeps hitting, hit again — and is written down.** The
first draft of that fix's comment contained the phrase "the shared `.bar` class at 960px", and
`rps_lint`'s ES6 pattern `(?<![\w.])class\s+[A-Za-z_$]` matched "class at" as a **class declaration**,
turning `RPS GATE` red on an ES5-clean file. Reworded to "the shared `.bar` selector at 960px". Noted
plainly because every PR in this initiative has tripped this at least once.

### The eight pages that needed nothing — what was actually checked

Each was loaded with its real data and measured at 960 and 720 CSS px; "clean" below means
`documentElement.scrollWidth == clientWidth` with no element extending past the viewport and no
unintended internal horizontal scroller.

- **`learn.html`** — a real quiz, then **all 10 questions answered**, score card shown, 10 citation
  links rendered. Clean at both. Single-column `max-width:760px` layout with nothing to collide.
- **`coverage.html`** — the real `/api/coverage` payload (39,683 documents, 1,848,465 pages, 9
  sidecar rows). Card grid reflows **3 columns → 2** on its own `auto-fill` track; sidecar table
  fits. Clean at both. *Noted, deliberately not fixed here:* `cad.pct` is **156.3%**, so that meter's
  inner `<i>` is 156% wide — clipped by the parent's `overflow:hidden`, identical at 1440px, and
  therefore a pre-existing data/clamping oddity rather than anything responsive.
- **`ingest.html`** — preview card with a real long Windows path, the air-gap `<details>` expanded,
  and a synthetic progress card carrying a **130-character monospace path** in `.current`. Clean at
  both: `base.css`'s `overflow-wrap:break-word` wraps that path to 3 lines with no page overflow, and
  the page's own `.bar` (which declares no `flex-wrap` of its own) is confirmed computing
  `flex-wrap: wrap` from the shared sheet. A direct, positive check that two of `[1.57.0]`'s rules do
  real work on a real page.
- **`ops.html`** — real `/api/ops` data, plus a 5-row per-vehicle coverage list and a **worst-case
  7-column run row** (long counts, a failure marker). The runs table measures **650px inside a 678px
  card** at 720px. Clean at both.
- **`verify.html`** — a 68-module chip roster and 8 database-integrity tiles. `auto-fill` tiles go
  **5 columns → 3**. Clean at both.
- **`command.html`** — real `/api/command_status` plus the real 13-row search-gaps table. Clean at
  both. *Noted, not fixed:* its `.cards` grid nested inside a `.row` flex shrink-wraps to a single
  150px column — measured identical at **1440, 1000 and 960px**, so pre-existing and not a narrow-
  window regression.
- **`collections.html`** — 6 collections plus a real one opened, rendering **120 result rows**. Grid
  **3 columns → 2**. Clean at both. Its `.cq` query lines report as internally-overflowing by design
  (`white-space:nowrap; overflow:hidden; text-overflow:ellipsis`).
- **`review.html`** — two realistic queue items with long subjects and full approve/override/reject
  control rows. Clean at both; `.submit` and `.ctl` already declared their own `flex-wrap`.

### `base.css` was deliberately not touched

Every defect above was specific to one page, so all three fixes live in that page's own inline
`<style>` (or, for `demo.html`, its own inline script). Three sibling batches of this same pass were
in flight against the same shared sheet, and a page-specific rule leaking into it is exactly the
collision that would have hurt. The new test asserts this directly: `#out`, `#niintbl` and `.tscroll`
must not appear in `base.css`.

### Tests

New `engine/tests/test_responsive_batch3.py` — **25 checks, all passing** — guarding all three fixes
plus the shared rules they lean on. It says up front what it can and cannot prove: this repo has no
headless browser, so it cannot re-measure a layout; it fails loudly if a fix is deleted, reverted, or
moved into the shared sheet. **Proven non-vacuous rather than asserted to be:** with all three fixes
deliberately reverted in place, the suite reported **18 passed, 7 failed, exit 1**, naming exactly the
reverted ones; the three files were then restored and confirmed **byte-identical** to their backups
(`diff` clean) and the suite returned to 25/25.

Real command output from this branch, every line actually run and seen:

- `python engine/tests/test_responsive_batch3.py` → **`25 passed, 0 failed`**; mutation run →
  **`18 passed, 7 failed`**, exit 1.
- `python engine/tests/rps_lint.py` → **`RPS GATE: PASS -- every ES5-required page is ES5-clean
  (10 modern-by-design pages noted)`** (after the `class at` rewording described above).
- `python engine/build_iteration_snapshot.py` → `R10 integrity OK`.
- `python engine/tests/verify_all.py --snapshot` → see the PR body for the full run and for the
  `test_ingest_routes.py` port-8894 contention this initiative has already documented twice
  (`[1.52.0]`, `[1.57.0]`), which three concurrent sibling worktrees make likely again.

---

## [1.59.0] — 2026-09-04 — Per-page responsive verification, batch 2 of 4: 12 pages resized for real (multi-window support, PR 9/25)

**VERSION → `1.59.0`.** Stage 3, PR 9 of `docs/superpowers/specs/2026-09-03-multi-window-tabs-plan.md`
— "PR 8-11: per-page verification, batched by the app's existing 6 home-nav section groupings."
`[1.57.0]`/PR 7 added the shared breakpoints to `base.css` and said so plainly: *not one real page
had been opened in a resized window yet.* This is that work for **one of the four batches**. Three
sibling batches are being built in parallel on this machine and have claimed `1.58.0`/`1.60.0`/
`1.61.0`; `1.59.0` was taken up front to keep the four from colliding, the same convention
`[1.53.0]`/`[1.57.0]` used.

**The 12 pages in this batch**, all opened against the running server and measured at **960 CSS px**
(half a 1080p monitor, `base.css`'s primary anchor) and **720 CSS px** (a docked or quarter-width
window): `solve.html`, `troubleshoot.html`, `ask.html`, `handover.html`, `circuitlab.html`,
`scan.html`, `semantic.html`, `visual.html`, `kg.html`, `related.html`, `index.html`, `help.html`.

**How they were actually checked, since "resized it and it looked fine" is not a result.** Each page
was loaded in a real browser at each width — with real content, not an empty shell: `solve` driven
through both stages for `ALTERNATOR` (5 cards), `troubleshoot` opened onto a fault tree that really
has checks (`will not start`, 25 symptoms, 2 steps), `ask` left to finish its ~25-second
`/api/ask` round trip (5 cited sentences), `semantic` at 15 real hits, `related` at 7 assemblies,
`kg` against a real graph node, `circuitlab` with the RLC sample loaded and simulating, `index`
past its side-gate with 30 search results and its in-app document viewer open on a real page.
Two instrumented passes ran on every one:

1. **An overflow probe** walking every rendered element: anything whose right edge passes the
   viewport, anything whose `scrollWidth` exceeds its `clientWidth` under `overflow-x:visible`, and
   anything clipped by more than 20px under `overflow-x:hidden` (a silent-content-loss case an
   ordinary overflow check misses entirely), plus `documentElement.scrollWidth` vs `clientWidth`.
2. **A mid-word-break detector**, added after the first real finding below: it records every leaf
   element's height, sets `document.body.style.overflowWrap='normal'`, re-measures, and reports
   every element that is *taller* with `base.css`'s shared `body{overflow-wrap:break-word}` than
   without — which is exactly the signature of a word being split where it should not be.

**Two pages needed a fix. Both are in that page's own inline `<style>`; `base.css` is untouched.**

**1. `index.html` — control labels split mid-word in the in-app document viewer (real, visible, at
720px).** `.vbar`'s fourth `.pgctl` group is the densest control row in the app: Clean, four range
sliders with their text labels, then Mirror / HD / Loupe / Callouts / Reset. It is a plain flex row
with no wrap, so below roughly 960px every control is flex-shrunk narrower than its own label — and
once `[1.57.0]`'s shared `body{overflow-wrap:break-word}` is active at the same width, four of those
labels break **inside the word**. Measured element heights at 720px, before: `contrast` 16 → 32px,
`zoom` 16 → 32px, and `↔ Mirror` / `🔎 Loupe` / `🏷 Callouts` / `⟲ Reset` 52 → 71px each, rendering
on screen as `Mirr / or`, `Loup / e`, `Callou / ts`, `Rese / t`. Worth being precise about the
cause: the row was **not** overflowing — `scrollWidth` and `clientWidth` were both 688px with and
without the rule — so no overflow check would ever have found this. `[1.57.0]`'s own entry called
this trade honestly ("a long part number can now wrap mid-string … PRs 8-11 can override it per page
where a specific identifier must stay intact"); this is the first page where that came due, and the
answer turned out to be better than an override. **Fix:** `@media (max-width:960px){.pgctl{flex-wrap:wrap}}`
— let the row wrap instead of crushing its own children. After: every button back at its natural
width (`Clean` 76, `Mirror` 74, `HD` 54, `Loupe` 80, `Callouts` 87, `Reset` 70) at a uniform **33px**
height with its label whole on one line, the group going from one 71px row to two rows totalling
89px. The toolbar costs **18px** of height (`.vbar` 249 → 267px) and buys back six readable controls,
and the detector reports **zero** mid-word breaks afterwards. It is an improvement at 960px too,
where the labels were already sitting on a second line under their icon (56px-tall buttons → 37px).
Scoped at 960 rather than this file's own long-standing 920px block, which keeps its separate job
(collapsing `main` and the `.vside` rail, both re-verified working at 720px in this pass) and is left
untouched. **Not put in `base.css`:** `.pgctl` is an `index.html`-only class name — `base.css`'s own
`pointer:coarse` rule already describes it as "index.html's in-viewer zoom/contrast/tilt row" — so a
single-page problem is fixed on the single page that has it.

**2. `handover.html` — a card that silently clips a too-wide table (latent, measured, not observed
with today's data).** `.card` is `overflow:hidden`, which is what clips its rounded corners around
the full-width `<table>` inside it. The side effect is that a table *wider* than the card is cut off
with **no scrollbar, no page-level overflow, and nothing on screen to say a column is missing**.
Measured directly at 720px: a table forced to 1299px sat inside a 670px card with 629px of it simply
gone. It is width-dependent — at desktop width the same table fits — so it appears only as a window
narrows, which is precisely the popped-out half-monitor case this pass exists for. **Fix:**
`@media (max-width:960px){.card{overflow-x:auto}}`. After: `overflow-x` is `auto` and `overflow-y`
stays `hidden`, the previously-lost 629px is reachable by scrolling, the rounded corners still clip,
and the page itself still has no horizontal overflow. **The honest scope, stated rather than
implied:** with today's data this is latent, not a break anyone has seen. Both *wired* tables
(pending sign-off, recent field notes) were measured at 720px with realistic rows — hyphenated NSNs,
a superseded `MS51922-17` part number, long names — and fit with room to spare (card `scrollWidth`
670 = `clientWidth` 670). The two that would hit it first are the conflicts and due-services tables,
which render raw `JSON.stringify` output — a near-unbroken token by construction, which
`overflow-wrap` cannot break because `overflow-wrap` does not affect a table column's min-content
width — and whose server side is not wired yet, as the notes under those two sections already say.
The fix is one declaration and zero-risk, so it was made rather than filed.

**The other ten pages needed nothing, and here is what that means concretely.** No horizontal
overflow, no element past the viewport, no clipped content, and no mid-word breaks at either width,
with real content loaded. Where the shared rules were doing the work, that was confirmed rather than
assumed:

- `solve.html` — its own `header`/`.bar` flex rows have no `flex-wrap` of their own and inherit
  `base.css`'s ≤960 rule; its `.grid` keeps its own 760px collapse. `945px`/`720px` document width,
  no overflow, through both stages with 5 cards rendered.
- `troubleshoot.html` — the fault-tree card, its wrapping button row and its step list all hold at
  720px; screenshot-checked as well as measured.
- `ask.html` — 5 real cited sentences with `.cite{white-space:nowrap}` badges inline; no overflow at
  either width.
- `circuitlab.html` **(the page flagged as needing extra care)** — the three-column `194px 1fr 236px`
  app shell still fits at 720px (stage 290px) and at 960px (stage 530px), with the RLC sample drawn
  and simulating. The SVG stage is **not** distorted or mis-tiled: its background grid `<rect>`
  measures exactly the stage width at both (290 = 290). One scare was chased to ground and is worth
  recording — a stale 530px grid rect was observed after resizing, which looked like a real
  mis-tile. It was an artefact of the test harness, not the page: CDP device-metrics emulation
  changes the viewport without firing a `resize` event, and `circuitlab.html` redraws on
  `window.addEventListener("resize", draw)`. Dispatching the event manually snapped the grid to
  970px immediately, and a fresh load at each width is correct. `base.css`'s deliberate exclusion of
  `svg`/`canvas` from its `max-width:100%` rule, and of `.wrap` from its `flex-wrap` rule (this page
  uses `.wrap` as a grid app shell), are both confirmed correct here.
- `scan.html` — `.wrap` is already `max-width:640px` and `.row` already wraps; the 4/3 camera stage
  is unaffected by the shared `max-width:100%` on `video`, which is inert against its
  `width:100%;height:100%;object-fit:cover`.
- `semantic.html` (15 rows), `visual.html`, `kg.html`, `related.html` — clean at both widths.
  `visual.html`'s `#preview{max-width:200px}` correctly beats the shared specificity-0
  `:where(img,…){max-width:100%}`, which is the intended outcome of PR 7's specificity choices.
- `help.html` — its own `@media(max-width:720px)` collapses `.grid` to a single `680px` column,
  measured; two columns between 721 and 960px, which is correct at that width.
- `index.html` (beyond the fix) — `main` is `564px 340px` at 960 and collapses to a single `680px`
  column at 720; the header wraps; **A1's pop-out `↗` buttons from `[1.55.0]` are fully on screen**
  inside the Tools dropdown at 720px (289px wide, left 138 / right 427, scrolling vertically under
  its own `max-height`); the side-gate modal fits (680px inside its `max-width:96vw`); and the
  in-viewer `.vside` rail correctly goes full-width with thumbnails hidden, per this file's own
  920px block.

**One real collision found and deliberately NOT fixed here, because it is not a responsive bug.**
The bottom-right pill cluster overlaps itself on every page: `#vw-read-btn` (readaloud.js) runs
458→524, `#bench-pill` 503→570, `#cmdk-pill` 552→708 — a 21px and an 18px overlap. It looks like a
narrow-window defect and is not: re-measured at **1500px** the identical overlap is present
(`#vw-read-btn` 1238→1304, `#bench-pill` 1283→1350, `#cmdk-pill` 1332→1488). It is width-independent,
pre-existing, and lives in shared chrome (`palette.js` + `readaloud.js`) that affects all 48 pages —
so fixing it inside one batch's PR, while three sibling batches are touching the same page family,
would be exactly the kind of shared-file change most likely to conflict. Recorded here so it is not
lost; it belongs in its own PR.

**Tests.** New `engine/tests/test_responsive_batch2.py` — **49 checks, 49 passed, 0 failed** — picked
up automatically by `verify_all.py`'s glob. It is deliberate about what it can and cannot prove: the
findings above are rendered-layout facts needing a real viewport and a real font, so the *browser*
measurements are the evidence and are quoted, while the file guards the structural half the design
spec calls "every markup-level change". It parses each page's inline `<style>` **with CSS comments
stripped first** — not cosmetic, since both fixes carry doc comments that name the very properties
they set, so a naive substring search would pass even if the real declaration were deleted and only
the prose left — brace-matches the `@media` blocks, and asserts: each fix exists, declares the
property that actually fixes the bug, is scoped to the breakpoint it was measured at and *not*
applied globally; `index.html`'s pre-existing 920px block still collapses `main`, widens `.vside` and
hides `.vthumbs` and was not absorbed into the new one; `handover.html`'s `.card` base rule still
carries `overflow:hidden` and the fix does not loosen `overflow-y`; all 12 pages still link
`/base.css` and still declare `<meta name="viewport" content="width=device-width…">` (without it a
narrow browser lays out at ~980px and scales, and every rule verified in this pass would silently
never fire — an invariant nothing else in the suite guards); and the eight pages that needed no fix
still have no page-local width breakpoint, while `solve`'s 760px and `help`'s 720px collapses are
still there. The suite was **negative-controlled**: with the two fixes programmatically removed it
returns `45 passed, 4 failed`, exit 1 — it fails for the right reasons, then the files were restored.

Real command output from this branch, every line actually run and seen:

- `python engine/tests/test_responsive_batch2.py` → **`49 passed, 0 failed`** (and `45 passed, 4
  failed`, exit 1, with the fixes removed).
- `python engine/tests/rps_lint.py` → **`RPS GATE: PASS -- every ES5-required page is ES5-clean (10
  modern-by-design pages noted)`**. Checked *before* touching anything, as required: of these 12
  pages only `solve.html` and `help.html` are `ES5_REQUIRED`; the other ten (including `index.html`
  and `circuitlab.html`) are `MODERN_BY_DESIGN`. In the event **no inline `<script>` was touched on
  any page** — both fixes are CSS — so the ES5 question never arose.
- `python engine/build_iteration_snapshot.py` → `iterations: 260 | legacy: 141 | latest: 1.59.0` ·
  **`R10 integrity OK -- all 260 changelog versions present in the snapshot.`**
- `python engine/tests/verify_all.py --snapshot` → **`67 checks | 67 ok | 0 FAILED` ·
  `ALL GREEN -- suites pass and every protected file matches the vault.`** All **64** `test_*.py`
  suites PASS, including the new `test_responsive_batch2.py` (0.1s), `test_uiux_fixes.py` at
  **273/273** (the suite that string-splits `base.css` itself — the direct check that this PR did not
  disturb the shared sheet, which it does not touch at all), and `test_routes.py` at **296 passed, 0
  failed** with no sign of the known pre-existing `/api/ask` timeout flake. Plus
  `rps_lint.py` → `RPS GATE: PASS` and
  `safeguard verify: 737 files, 737 OK, 0 DAMAGED`.

**Notes on the run itself, since this initiative has kept a habit of writing these down.** It went
green on the **first** attempt — no repeats, nothing re-run until it passed. Free disk was checked
before starting (**38.5 GB** on `C:`) after a sibling PR hit a false-failure cascade from a nearly
full drive, and **port 8894 was confirmed free** before starting, because
`test_ingest_routes.py` binds it unconditionally and `[1.52.0]`/`[1.57.0]` both documented a
confusing `IndexError: list index out of range` on `_popen_calls[0]` when a *different worktree's*
copy of that suite already holds the port (`allow_reuse_address` lets the second bind succeed on
Windows, so requests reach the other process). With three sibling batches running the same suite
concurrently tonight that was a live risk; it did not fire, and `test_ingest_routes.py` PASSed
(37.2s). As in `[1.57.0]`, the docs edits and the
`ITERATION-SNAPSHOTS.md`/`ITERATION-DASHBOARD.html` regeneration that record this run necessarily
happened *after* it — writing a result into the repo modifies the tree it just verified.

**The confirmatory post-edit run on the finished tree came back `67 checks | 66 ok | 1 FAILED`, and
that one failure is reported rather than buried.** It is `test_routes.py`, on exactly one line:
`FAIL GET /api/ask?q=how%20to%20remove%20the%20brake -> request error: timed out` — the same known,
pre-existing `/api/ask` timeout flake `[1.57.0]` names, and the one this PR happened to measure
independently while checking `ask.html`, where a real `/api/ask` round trip took roughly **25-30
seconds** in this environment against the test's shorter HTTP timeout. It is not this change's
content (both fixes are CSS, and `ask.html` is one of the ten pages that needed no fix), it passed
296/296 inside the run that *did* go fully green, and running it alone immediately afterwards gave
**`296 passed, 0 failed`** again. Machine load is the plausible difference: three sibling batches
were running the same suite concurrently. `safeguard verify` was clean in both runs
(`737 files, 737 OK, 0 DAMAGED`), and `rps_lint`, `test_responsive_batch2.py`,
`test_ingest_routes.py` and `test_uiux_fixes.py` all PASSed in both.
...
**VERSION → `1.58.0`.** (Claimed up front: three sibling batches of this same pass are being built in
parallel and have claimed `1.59.0`/`1.60.0`/`1.61.0`, so this branch takes the lowest free number
rather than racing for one — the same convention `[1.57.0]`/`[1.53.0]`/`[1.52.0]` used.)

Stage 3, PR 8 of `docs/superpowers/specs/2026-09-03-multi-window-tabs-plan.md` — the first of the four
per-page verification batches `[1.57.0]` deliberately left undone. `[1.57.0]` shipped the shared rules
and said plainly that **not one real page had been opened in a resized window**. This PR does that for
13 of them: `part` · `procedure` · `torque` · `jobcard` · `bench` · `dossier` · `partdiff` · `locate` ·
`decode` · `fastener` · `pmcs` · `measures` · `readiness`. The first five land here specifically
because PR 14 (A2, the per-page pop-out control) is blocked on them.

**How they were checked, since "verified" is the word most likely to be doing no work here.** The real
server was started against the real 227,908-row corpus and each page was loaded with a query that
actually returns data (`part`/`torque`/`locate`/`measures`/`jobcard`/`partdiff`/`dossier` on
`alternator` and NSN `3040-01-521-7377`, `procedure` on `brake`, `pmcs` on `5 TON`, `decode` on
`5310-01-359-2198`, `fastener`'s static table as shipped) — not empty shells, which is the easy way to
"verify" a responsive pass that has found nothing. Each was then measured in a real browser at 960px
and 720px, and swept down to 360px, with a probe that walks every non-fixed element in `body`, records
any whose right edge passes the viewport, and separately records any whose own `scrollWidth` exceeds
its `clientWidth`. `readiness`'s `/api/fluids`/`/api/intervals` and `measures`'s `/api/external`
return nothing on this machine (that data is not built / needs the open internet), so those two paths
were exercised with stubbed responses of the exact documented shape rather than reported as passing on
an empty page.

**One thing worth recording because it nearly produced a false pass:** the server holds UI files in
memory after first read, so the first post-edit measurement showed the fix having no effect at all. It
was not a bad fix — the browser was being served the pre-edit file. Every "after" number below comes
from a server restarted on the edited tree, confirmed by `curl`-ing the page and grepping for the new
rule before measuring.

### Found and fixed

**`procedure.html` — the reference rail wrapped but kept its two-column width (721-755px).** This is
the one `[1.57.0]` predicted ("a rough ~20px band between its own natural wrap threshold and this PR's
720px rule") and it is real, larger than predicted, and now closed. `.side{width:420px;max-width:46vw}`
sits next to `.steps{flex:1;min-width:340px}` in a `.cols` flex row that `base.css` makes wrap at
≤960px. Measured, one width at a time: **756px is the last width where the two sit side by side**
(steps 348 / side 348). At **755px** the row wraps — and wrapping makes the page taller, which brings
in a 15px vertical scrollbar, which drops the usable width to 740px and holds it wrapped. That is a
stable equilibrium, reproducible on every reload, not a flicker. The defect is what the rail does once
wrapped: it keeps its declared `420px`/`46vw` and lands **332-347px wide inside a 677-696px row**, so
the scanned manual page a technician is checking the steps against renders at under half the width
sitting empty beside it — strictly worse than either neighbouring state. Pre-fix that held from 755px
down to 721px, where `base.css`'s shared `body .side{width:100%;max-width:none}` finally took over.
Fixed with one page-local rule, `@media(max-width:755px){ .side{width:100%;max-width:none} }`.
**After:** 756px unchanged (348/348 side by side, and 480/420 at 960px); 755px→721px the rail is now
**677-711px** wide instead of 332-347px; zero horizontal overflow at every width tested.
**The plan's own ~20px estimate was 15px short** — it was computed from the layout arithmetic alone and
did not account for the scrollbar the wrap itself brings in. 755px is the measured wrap point, so that
is the threshold used, not a rounder guess.

**`measures.html` — `.m` and `.em` are non-wrapping flex rows that scrolled the page sideways from
490px down.** Neither name is in `base.css`'s shared
`header,.bar,.row,.cols,.search,.chips,.toolbar,.tools,.tabs{flex-wrap:wrap}` list, and correctly so:
those are shared class names, these two are this page's own — adding them to the shared sheet is
exactly the mistake `[1.57.0]`'s entry warned about for `.grid`. Their content floor is real: `.val`'s
`min-width:120px`, `.ty`, `.ctx`, a `white-space:nowrap` trust chip, the trailing `p.N ↗` citation link
and four 12px gaps come to ~411px. Measured: the page first overflows at **490px** (1px), **480px**
(11px), **375px** (116px), with the citation link — the one control on the row a technician has to
reach — pushed off the right edge. Fixed by adding `flex-wrap:wrap` to both, the same rule `base.css`
applies to the shared row classes. **This is below this batch's own 960/720 anchors, and is called out
as such rather than dressed up as an anchor-width find** — it is fixed anyway because 480px is a
quarter of the same 1080p monitor the 960px anchor is half of, and because the fix is one declaration
that provably changes nothing above 491px.

**Both fixes live in the page's own inline `<style>`. `engine/ui/base.css` is not touched by this PR**
— three sibling batches of this same pass are in flight, and neither of these is a shared-layer
problem.

### Checked and genuinely needed nothing (11 pages)

Not a blanket claim — what was actually looked at, per page, at 960px and 720px, with real data
loaded: `part.html` (its `.grid2` correctly collapsed by `base.css`'s `body .grid2` rule — measured
`913px` single column at 960px — plus 2 tables and a torque-sequence SVG; clean all the way to 360px),
`torque.html`, `jobcard.html` (its own `@media(max-width:720px)` `.grid` collapse fires correctly),
`bench.html` (seeded with 4 real pinned rows first, since an empty bench proves nothing),
`dossier.html`, `partdiff.html` (its `auto-fill` `minmax(320px,1fr)` grid reflows on its own, as
`[1.57.0]` predicted when it declined to touch `.grid`), `locate.html`, `decode.html`, `pmcs.html`,
`readiness.html` (stubbed fluids + intervals tables), `fastener.html`. All eleven: `scrollWidth` equals
`clientWidth` at 960px and 720px, no element past the right edge, no element with internal horizontal
overflow.

**Also checked, because the app has a touch tier and the shared sheet has a `pointer:coarse` block that
inflates every control to 44×44:** all 13 pages were measured at 720px *with* device emulation on
(`pointer:coarse` matching, `min-width:44px` confirmed live on a real button), and again at 960px with
coarse pointer forced, which is the case that matters because `jobcard`'s and `dossier`'s two-column
grids are still live at 960px while collapsed at 720px. Zero overflow in every combination.

### Found, measured, and deliberately NOT fixed here

- **The bottom-right fixed chrome overlaps itself — but it is not a responsive bug.** `#cmdk-pill`,
  `#bench-pill`, `#vw-footer` and `#vw-read-btn` overlap by 18×44, 156×4, 66×4 and 21×29 pixels.
  Measured at 1400px, 960px and 720px on a desktop pointer: **the four overlap rectangles are
  byte-identical at all three widths**, so it is pre-existing and width-independent, present at full
  desktop size. (With a coarse pointer the overlaps are still there; touch sizing only grows the last
  pair, 21×29 → 21×44 — it does not create them.) It comes from
  `palette.js`/`readaloud.js`/`base.css` chrome shared by all 48 pages rather
  than from anything in these 13. Recorded here so the next person does not have to re-measure it;
  fixing it belongs in its own PR, not in one of four parallel batches all touching the same files.
- **`procedure.html` overflows by 17px at 360px**, from its own deliberate `.steps{min-width:340px}` —
  the exact floor `[1.57.0]` chose `:where()` specificity-0 in order to leave alone. Below any named
  scenario, and changing it would fight a decision the page made on purpose.
- **`fastener.html`'s 5-column reference table overflows by 26px at 360px.** Also below any named
  scenario; a 5-column geometry table at phone-portrait width is a design question, not a wrap rule.

### Tests

`engine/tests/test_uiux_fixes.py` gains **12 checks** (273 → 285) guarding both fixes: that
`procedure.html` carries its own `.side` breakpoint and still declares the `420px`/`46vw` rail it
overrides, that `base.css` still owns the shared 720px default the page deliberately overrides, that
the page-local threshold number **read back out of both files** sits above the shared one (not a
restated constant), that `.m`/`.em` are still flex rows and still declare `flex-wrap`, and that neither
page-local class has been quietly absorbed into `base.css`'s shared wrap list. These are source-text
assertions, **not layout measurements** — this suite has no browser, and the layout evidence is the
before/after numbers above. They were negative-tested: reverting both fixes turns 285/0 into **280
passed, 5 failed**, and restoring them returns 285/0.

Real command output from this branch:

- `python engine/tests/rps_lint.py` → `RPS GATE: PASS -- every ES5-required page is ES5-clean (10
  modern-by-design pages noted)`. No inline `<script>` was touched on any of the 13 pages; both fixes
  are CSS. `procedure.html` is ES5-required and stays ES5-clean; `measures.html` is modern-by-design.
...
`engine/ui/*.html` file is touched, and **not one real page has been checked against these rules in
a resized window yet.** That is explicitly PRs 8-11, batched by the home nav's own 6 section
groupings, where real overflow and collision get found and fixed page by page. Everything below is
the foundation those four PRs inherit, and nothing more — said plainly so the "responsive baseline"
line item is not read as finished when it is one PR of five.

**The premise, checked rather than taken from the spec.** The spec claims zero width-based
breakpoints exist in `base.css`. Verified directly: before this change the file's only media queries
were `(pointer:coarse)`, `print` and `(prefers-reduced-motion: reduce)` — three, none width-based.
So the one sheet 48 of 48 pages `<link>` contributed *literally nothing* to a narrow window. Eight
pages had since grown their own ad-hoc width breakpoints, at seven different numbers
(1280/920/820/780/760/720/620 — `index.html`, `dossier`, `exploded`, `help`, `jobcard`, `part`,
`solve`, `procedure`'s print rule); the other forty had none at all.

**Two anchors, neither invented.** `960px` is exactly half of a 1080p monitor — the concrete
scenario the spec names, and the one `[1.53.0]`'s `VW.windows` makes ordinary rather than
hypothetical (the whole point of a pop-out is reading `torque.html` at ~960 CSS px instead of
~1900). `720px` — a docked or quarter-width window — is the number four of this app's own pages
already picked for themselves (`help`/`jobcard`/`solve` at 720-760, `part` at 760), so the shared
sheet agrees with the pages instead of fighting them.

**The part that would have made this whole PR inert, and how it was avoided.** `base.css` is
`<link>`ed in `<head>` *before* every page's own inline `<style>` block, and a media query adds no
specificity. A plain `@media(max-width:960px){.grid2{...}}` written in `base.css` therefore **loses**
to `part.html`'s later, equal-specificity `.grid2{grid-template-columns:1fr 1fr}` — the rule would
parse, match, and be overridden, and the sheet would look correct while doing nothing. Each of the
seven rules added here picks its weight deliberately: `:where(...)` (specificity 0) for the safety
nets any page must stay free to override, a bare element/class selector where no page declares that
property at all, and `body .x` (one notch up) *only* where the rule genuinely has to beat a page's
own declaration. This is verified below, not asserted.

**What was added** — one unbreakpointed rule and six inside two media queries:

- `#vw-toast{max-width:calc(100vw - 24px)}` — deliberately *not* inside a breakpoint: it is
  `base.css`'s own chrome (no page defines `#vw-toast`) and the constraint is self-limiting, binding
  only once the viewport is actually narrower than the toast. `[1.53.0]` made toasts routine on this
  exact path — "Already open — switched to that window" fires on every pop-out reuse — and the
  longest of those strings is wider than a narrow popped-out window.
- **≤960px** — `header,.bar,.row,.cols,.search,.chips,.toolbar,.tools,.tabs{flex-wrap:wrap}`. Not a
  new convention: this app's own, finished. Counted across the page family, `.search` already
  declares `flex-wrap:wrap` in **18 of its 18** flex definitions, `.toolbar` 2/2, `.chips` 2/2,
  `.cols` 1/1, `.tools` 1/1, `.row` 7/8, `.bar` 8/14, `header` 5/9, `.tabs` 0/1. The pages that
  forgot are precisely the ones that break when squeezed. `flex-wrap` is inert on a non-flex element
  (several pages reuse `.bar` for a progress meter and `.tools` for a plain block) and inert on a
  row that still fits. `.wrap` is deliberately excluded — `circuitlab.html` and `deepzoom.html` use
  it as a full-height flex/grid app shell, where wrapping a column-direction container is meaningful.
- **≤960px** — `:where(.grid,.grid2,.cards,.tiles,.cols,.bar,.row,.search)>*{min-width:0}`. Grid and
  flex items default to `min-width:auto`, so one wide table or one long unbreakable token inside a
  column forces the whole track wider than its container and scrolls the page sideways. This is not
  a theoretical failure: `index.html` hit it and worked around it locally *twice* — its
  `grid-template-columns:minmax(0,1fr) 420px` carries the comment "the results column can never force
  sideways overflow", and its own 920px block sets `.vside{min-width:0}`. `:where()` keeps this at
  specificity 0 so a page's deliberate floor still wins; `procedure.html`'s
  `.steps{flex:1;min-width:340px}` is untouched, which is the intended outcome rather than a gap.
- **≤960px** — `body{overflow-wrap:break-word}`. NSNs, CAGE codes, part numbers and file paths are
  what this app is made of, and a single unbreakable one forces a horizontal scrollbar. Inherited,
  so one declaration covers table cells (18 pages carry `<table>`) and card bodies alike;
  `break-word` only ever breaks a word that could not fit a line on its own. No page sets
  `overflow-wrap`/`word-wrap`/`word-break` anywhere, so a bare `body` selector is enough. **Honest
  trade:** in a narrow window a long part number can now wrap mid-string. That is worse to read than
  an unbroken one and better than a page that scrolls sideways; PRs 8-11 can override it per page
  where a specific identifier must stay intact.
- **≤960px** — `:where(img,video,iframe){max-width:100%}`. Specificity 0, so pages that size images
  explicitly (thumbnail strips at `width:84px`, `object-fit` covers) keep their own sizing. `svg` and
  `canvas` are deliberately excluded: the 3-D/deep-zoom/circuit stages size theirs by script.
- **≤960px** — `body .grid2{grid-template-columns:1fr}`. The one class in this app actually *named*
  for being a two-column split (`part.html`, `1fr 1fr` with its own 760px collapse) collapses at the
  half-monitor width instead. `body .` because it has to beat that page's later declaration.
- **≤720px** — `body .side{width:100%;max-width:none}`. `procedure.html`'s
  `.side{width:420px;max-width:46vw}` next to `.steps{min-width:340px}` is this app's only
  `.side`-named split today: below roughly 740px the two stop fitting, the wrapping `.cols` row drops
  the rail underneath, and it lands there still only ~330px wide — worse than either layout on its
  own. One consumer today; PRs 8-11 may find more, and this is the shared rule they inherit.

**What was deliberately NOT done, and why.** `.grid` is *not* collapsed to a single column, which is
the obvious rule to reach for and would have been wrong. It means two different things across this
app: an explicit two-column `1fr 1fr` split on 5 pages (`dossier`/`help`/`jobcard`/`solve`/`exploded`
— every one of which already collapses itself at 720-820px) and a
`repeat(auto-fill,minmax(150-320px,1fr))` card grid on 6 others
(`collections`/`coverage`/`schematics`/`threed`/`partdiff`/`demo`) that already reflows correctly on
its own. A blanket collapse fixes nothing on the first group and turns the second into a column of
900px-wide cards. That judgement needs a real resized window per page. Also considered and dropped:
tightening `.wrap`'s horizontal padding (44 of 48 pages use `.wrap`, but two of them use it as a
full-viewport app shell with no padding at all, so a blanket override in the shared sheet would
silently change those two to buy ~12px on the other 42). And `procedure.html` still has a rough
~20px band between its own natural wrap threshold (~740px) and this PR's 720px rule — real, known,
and exactly the kind of thing PR 8-11's per-page pass exists to tune.

**Verified for real, in a real browser, before/after.** There is no CSS linter in this repo, so the
file was checked three ways rather than eyeballed. (1) Brace balance with comments stripped: `{` 57 ·
`}` 57, nesting depth never negative, final depth 0, `/*` 31 = `*/` 31. (2) A real browser was made
to *parse* the file — the whole of `base.css` inlined into a `<style>` and read back through
`document.styleSheets[…].cssRules`, which silently drops anything it cannot parse: **43 top-level
rules**, and all five media rules present with every inner rule intact — `(pointer: coarse)` 4,
`print` 1, `(prefers-reduced-motion: reduce)` 1, **`(max-width: 960px)` 5**, **`(max-width: 720px)`
1**. Nothing was dropped, `:where()` included. (3) A cascade harness reproducing the real load order
(this `base.css` in one `<style>`, then a second `<style>` holding verbatim copies of `part.html`'s
`.grid2`, `procedure.html`'s `.cols`/`.steps`/`.side`, `solve.html`'s `header`/`.bar` and
`collections.html`'s `.grid`), served over HTTP and measured with `getComputedStyle`. The real
measured values, at 1200px → 960px → 720px:

- `header` / `.bar` `flex-wrap`: `nowrap` → `wrap`.
- `.grid2` columns: `593px 593px` → `960px` (collapsed) → `720px`; its child's `min-width`
  `auto` → `0px`.
- `.grid` (the `auto-fill` kind, from `collections.html`): 4 columns → **3 columns** → 2 columns —
  i.e. deliberately untouched by this PR at every width.
- `.steps` `min-width` (the page's own floor): `340px` → **`340px`** → **`340px`** — untouched, as
  the specificity-0 `:where()` choice intends.
- `.side`: `420px` / `max-width:552px` → `420px` / `441.6px` → **`720px` / `max-width:none`**.
- `body` `overflow-wrap`: `normal` → `break-word`.
- `img.thumb`: `max-width` `none` → `100%`, while `width` stays **`84px`** at both.

Three things those numbers are there to prove, and do: at **1200px every value is byte-identical to
what it was before this change** (the block is completely inert above 960px — R1); at 960px the
`auto-fill` `.grid` is still 3 columns and `.steps` still has its intentional 340px floor, i.e. the
`:where()` specificity-0 choice really does let pages win where they should; and the `body .`
selectors really do beat a page's later declaration where they must (`.grid2`, `.side`). Finally,
the overflow claim measured rather than asserted — same harness at **400px** wide:
`documentElement.scrollWidth` **400** = `clientWidth` **400**, no horizontal overflow; with the
`base.css` `<style>` toggled `disabled=true` on the same live page, the identical markup immediately
overflows to `scrollWidth` **534** against a 400px client. The rules are doing the work, not the
markup.

**What this still cannot prove, stated plainly.** Everything above is a synthetic harness holding
copies of four pages' rules — it demonstrates that the shared block behaves as designed against those
patterns, and nothing at all about whether the other 44 real pages look right at 960px. Only a human
resizing each one does that, which is PRs 8-11, and this entry should not be read as having done it.

Real command output from this branch, every line of it actually run and seen:

- Brace/comment audit of `engine/ui/base.css` → `{ 57 } 57` · `final depth 0` · `min depth 0` ·
  `/*=31 */=31`.
- Browser parse of the whole file → `topLevelRules: 43`, media rules
  `(pointer: coarse)`:4 · `print`:1 · `(prefers-reduced-motion: reduce)`:1 · `(max-width: 960px)`:5 ·
  `(max-width: 720px)`:1.
- `python engine/build_iteration_snapshot.py` → `R10 integrity OK -- all 257 changelog versions
  present in the snapshot.`
- Free disk checked before the suite ran (42.5 GB on `C:`), after a sibling PR hit a real
  false-failure cascade from a nearly-full drive.
- `python engine/tests/verify_all.py --snapshot` — the final full run:
  **`64 checks | 64 ok | 0 FAILED` · `ALL GREEN -- suites pass and every protected file matches the
  vault.`** All 61 `test_*.py` suites PASS, including the four that reference `engine/ui/base.css`
  — `test_uiux_fixes.py` **273 passed, 0 failed** (it string-splits this very file to assert on the
  kiosk-mode and `pointer:coarse` blocks, so it is the direct check that inserting a new section did
  not disturb them), `test_hardening.py` (`/base.css` served), `test_demo_tour.py`,
  `test_readaloud_stepnav.py` — plus `rps_lint.py` → `RPS GATE: PASS -- every ES5-required page is
  ES5-clean (10 modern-by-design pages noted)`, `test_routes.py` PASS (no sign of the known
  pre-existing `/api/ask` timeout flake), and `safeguard verify: 734 files, 734 OK, 0 DAMAGED`.

**Getting to that took three full runs, and both intermediate failures are written down rather than
re-run until green — neither was this change's content, and both were chased to root cause.**

1. **Run 1** — every suite PASS, `64 checks | 63 ok | 1 FAILED` on `safeguard verify`, reporting
   `734 files, 731 OK, 3 DAMAGED`: `engine/ui/base.css`, `docs/HANDOFF-NOTE.md` and
   `docs/MASTER-RECONCILIATION.md` as `MODIFIED (grew / edited)`. Self-inflicted ordering:
   `--snapshot` baselines at the *start* of a ~3.5-minute run, and those three files were still
   being edited while it was in flight. Nothing truncated or shrunk — all three *grew*, by exactly
   the bytes of the edits made.
2. **Run 2** — every suite PASS again, but `safeguard verify` crashed outright with
   `FileNotFoundError: ...\backups\vault\SNAP_20260903_230927_pre-ingest\manifest.json`. Root cause
   found rather than guessed: `verify_all.py` calls `safeguard.py verify` with **no** snapshot id,
   so `safeguard.latest_snapid()` picks the lexicographically-last `SNAP_*` directory — which after
   a full run is one the *test suites themselves* created (`ingest_feature.py` takes a real
   `safeguard.snapshot("pre-ingest")` before every ingest, and `test_ingest_routes.py` drives it).
   That is long-standing, documented behaviour, not new — `[1.53.0]`'s own entry records
   `verify vs SNAP_20260903_181820_pre-ingest`. What was new is that one of those test-created
   snapshots had a `files/` directory but no `manifest.json`, left behind by an aborted launch. All
   of this lives in `backups/`, which is **gitignored and not part of this PR at all**. Proof this
   PR's content is sound regardless:
   `python engine/safeguard.py verify --snap SNAP_20260903_230713_verify_all` (run 2's *own*
   baseline) → **`734 files, 734 OK, 0 DAMAGED`**. The manifest-less directory was then removed as
   the garbage it was, and a plain `safeguard.py verify` immediately returned
   `verify vs SNAP_20260903_230920_pre-ingest: 734 files, 734 OK, 0 DAMAGED`.
3. **Run 3** — `64 checks | 62 ok | 2 FAILED`: `test_routes.py` (the known pre-existing
   `FAIL GET /api/ask?q=... -> request error: timed out`, 295 passed / 1 failed) and
   `test_ingest_routes.py` with `IndexError: list index out of range` on `_popen_calls[0]`. The
   second one was traced, not assumed: `test_ingest_routes.py` stands up its own server on the
   **hard-coded port 8894**, and `netstat` showed 8894 already `LISTENING`, held by a
   `tests/test_ingest_routes.py` process whose parent shell was running out of **a different
   worktree entirely** — one of the sibling PRs being built in parallel on this machine, looping
   that same suite. `HTTPServer.allow_reuse_address` lets the second bind succeed on Windows, so the
   requests reached the *other* process, whose `subprocess.Popen` is not the one this run
   monkeypatched — hence an empty `_popen_calls`. This is precisely the false-failure mode
   `[1.52.0]` already documented for this exact file ("checks depend on process-global state and a
   fixed port"). Their processes were **left alone**; once port 8894 was free,
   `python engine/tests/test_ingest_routes.py` standalone → **`175 passed, 0 failed`**, and the
   final full run above came back ALL GREEN with `test_ingest_routes.py` PASS (34.2s).

The docs edits and the `ITERATION-SNAPSHOTS.md`/`ITERATION-DASHBOARD.html` regeneration that record
