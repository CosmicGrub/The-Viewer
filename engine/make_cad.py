...
def _render_one(task):
    """Render+cache one part. Runs in a worker process. Returns ('done'|'fail', nsn)."""
    nsn, name, chars, style = task
    try:
        p = cad_render.ensure(nsn, name, chars, CDIR, style=style)
        return ("done" if p else "fail", nsn)
    except Exception:
        return ("fail", nsn)
...
def main():
    ...
    if workers == 1:
        ...
        with Pool(processes=workers) as pool:
            for i, (st, _) in enumerate(pool.imap_unordered(_render_one, todo, chunksize=16), 1):
                if st == "done": done += 1
                else: failed += 1
                if i % 200 == 0 or i == n: _progress(i)
