...
**State: v1.77.0 · 2026-09-05** (rewritten 2026-08-08 from ~130 versions of drift, updated 2026-08-09,
...
55. **`[1.73.0]` — `VW.capabilities`: centralized feature-detection + tier registry (multi-window
    support, PR 19/25, stage 6).** "Depends on nothing" per the plan's own words — the first of six
    bleeding-edge-capability PRs (19–24) closing out this initiative. Adds a single object exposing
    `{tier, broadcastChannel, windowPlacement, wakeLock, pictureInPicture, fileSystemAccess, webLocks,
    indexedDB}` to `shared.js`, alongside the existing `VW.channel`/`VW.workspace`/`VW.windows`/
    `VW.bench`/`VW.checkpoint` — nothing else changes; per the plan's own text those existing
    namespaces and item 50/PR 17's `_screenPlacementAvailable()` stay byte-for-byte what they were.
    **The live-read decision, the one thing most worth explaining:** the plan doc calls this "computed
    once," which reads naturally as "cache the whole object at `shared.js` load time" — actively wrong
    here, because `window.RPS.mode` (the tier signal every non-tier field is AND-ed against) is set
    asynchronously by `rps.js`'s `boot()`, well after `shared.js`'s own top-level code has already run
    on most pages (only 17 of this app's 49 pages load `rps.js` at all). A one-time snapshot would
    silently lock every AND-ed-with-tier flag to a stale value forever, even after the real tier
    becomes known moments later — the same "live vs. cached" question item 49/PR 6's
    `windowsRegistry()` already answered (live reads off the window handle at call time, never a
    stale open-time snapshot). Every field here is therefore a LIVE getter (`Object.defineProperty`,
    plain ES5, never the newer getter/setter object-literal shorthand `rps_lint.py`'s ES6-syntax scan
    would flag), re-evaluated fresh on every read — "computed once" read as "the CHECK is written
    once," never "the VALUE is captured once." Every non-tier flag AND-s its raw browser-feature check
    with `tier === "modern"` EXACTLY (a strict string match, never truthy — `"premium"` never unlocks
    anything on its own, matching `_screenPlacementAvailable()`'s own established convention);
    `windowPlacement` calls that exact existing function directly rather than re-typing a second,
    drift-prone copy of its raw `getScreenDetails`/tier check. Every getter is independently
    try/caught so one hostile or locked-down raw check degrades only its own flag to `false`, never
    breaking any other flag's read. **Already-wired, previously-inert callers come alive with zero
    code changes:** item 46/PR 15's `jobcard.html`/`solve.html` (`launchWorkOrder()`/`launchSolveIt()`)
    were written reading `window.VW && VW.capabilities`/`caps.tier` back when they shipped,
    deliberately inert with their own "the day PR 19 ships ... no change needed here" comment — this
    PR makes that real, confirmed by a `git diff` assertion in the new tests that this PR's own diff
    touches neither HTML file at all. **Placement discipline:** per item 45's own `test_a2_popout.py`
    (A2), which slices `popoutControl()`'s body up to the next `var VW = {` marker, the new code lands
    entirely BEFORE `popoutControl()`'s own section, confirmed by a source-offset assertion plus a
    clean, unchanged re-run of `test_a2_popout.py` itself (62/0). New
    `engine/tests/test_vw_capabilities.py` + `engine/tests/js/test_vw_capabilities_node.js`, **82 real
    assertions** (17 top-level + 65 in the node `vm.createContext` behavioral layer, same sandbox
    convention item 49/`test_windows_layout.py` established): the live-read guarantee proven by
    mutating `window.RPS.mode` on an already-loaded sandbox (no reload) and confirming the next read
    reflects it, then flipping back and re-confirming; `window.RPS` entirely absent reads tier `"modern"`, never throws; every one of the
    7 AND-ed flags tested across present+modern/lite/legacy/premium and absent+modern (35 assertions,
    the `"premium"`-is-not-`"modern"` edge case included per flag); a throwing raw feature check
    (`window.documentPictureInPicture` given a throwing accessor — chosen because a Node vm sandbox's
    own global-object accessor semantics silently swallow a throw placed directly on the sandbox
    itself into a plain `ReferenceError` rather than propagating it, a real Node vm quirk confirmed by
    hand while building this suite, not a `shared.js` bug) degrading only its own flag, proven not to
    affect three other flags in the same object, and proven not to get cached (read twice, `false`
    both times); `windowPlacement`'s getter body asserted, at the source level, to call
    `_screenPlacementAvailable()` directly with no second `getScreenDetails` check of its own, and its
    live value asserted to match that exact expression across 6 scenarios; `VW.channel`/
    `VW.workspace`/`VW.windows` still export exactly what they did before this diff. **Proven
    load-bearing by breaking 3 representative guarantees one at a time** in the working tree —
    replacing the live tier getter with a value captured once at load time (2 failures, exactly the
    two live-read checks); removing one getter's `try`/`catch` (3 failures, exactly the three
    throw-isolation checks); re-typing `windowPlacement` as an independent copy of
    `_screenPlacementAvailable()`'s own expression instead of calling it (2 failures, at BOTH the node
...
56. **`[1.74.0]` — `VW.locks`: Web Locks API wrapper (multi-window support, PR 20/25, stage 6).**
    Depends on item 55/PR 19's `VW.capabilities.webLocks` — the FIRST real consumer of
    ...
    of duplicated-logic-that-can-drift item 55/PR 19 was built to rule out. `withLock` always returns a
    Promise settling with whatever `fn()` itself settles with (`fn` may return a plain value or a
    Promise, the same duck-typing `navigator.locks.request()`'s own callback return value already
    supports). **Real-API path** (webLocks true): delegates straight to `navigator.locks.request(name,
    function (lock) { return fn(); })` — the browser's own implementation already handles acquisition,
    release-on-settle, and cross-tab serialization; nothing here reimplements any of that. **Fallback
    path** (webLocks false — `lite`/`legacy` tier, raw API absent, or a non-modern tier even with the
    raw API technically present): a best-effort in-memory PROMISE-CHAIN MUTEX scoped to just this tab —
    not a real cross-tab guarantee, but "never blocks" (the design doc's own named edge case:
    ...
    internal `_lockQueues` map keys a lock name to its current "tail" promise: a call with a new name
    chains off `Promise.resolve()` (runs on the next microtask, "never blocks"); a call sharing an
    EXISTING name chains its own `fn()` invocation onto that tail, so same-name calls run strictly one
    at a time, in call order — genuine same-tab mutual exclusion; different names never share a tail,
    so they never wait on each other. The map's stored tail is always a SETTLED-REGARDLESS-OF-OUTCOME
    derivative of each call's own result (`.then(onFulfilled, onRejected)` with both handlers returning
    normally) — so a `fn()` that rejects or throws still lets the next queued call for that name run
    once it settles; a failure can never permanently jam a name's queue. The promise handed back to the
    CALLER, by contrast, is built directly off `fn()`'s real result, so the original rejection still
    reaches whoever made that specific call — only the internal chain-advancing copy swallows it, never
    the value the caller sees. Once a name's queue drains (no newer call replaced the tail in the
    meantime), its map entry is deleted, so `_lockQueues` never grows without bound across a long
    session — a debug-only `_debugPendingCount()` introspection hook (deliberately NOT part of the
    documented `VW.locks` API surface, which names exactly one member, `withLock`) makes this provable
    with a real executed assertion rather than a source-text guess. **Placement discipline:** per item
    46's own `test_a2_popout.py` (A2), which slices `popoutControl()`'s body up to the next
    `var VW = {` marker, the new code lands entirely BEFORE `popoutControl()`'s own section — directly
    after `VW.capabilities`, the same place item 55/PR 19 landed — confirmed by a source-offset
    assertion plus a clean, unchanged re-run of `test_a2_popout.py` itself (62/0). New
    `engine/tests/test_vw_locks.py` + `engine/tests/js/test_vw_locks_node.js`, **55 real assertions**
    (18 top-level + 37 in the node `vm.createContext` behavioral layer, same sandbox convention item
    55/`test_vw_capabilities_node.js` established): the real-API path genuinely delegates to
    `navigator.locks.request()` with the exact lock name, and fn()'s resolved/rejected value — as a
    plain value AND as a Promise — round-trips correctly both directions; the raw-API-absent case
    proven via the absence of the synchronous throw a real access against `undefined` would cause;
    **THE SINGLE MOST IMPORTANT TEST** — raw `navigator.locks` genuinely present, wired to a canary,
    but tier `"lite"`/`"legacy"`/`"premium"` — proves the fallback still runs and the canary is NEVER
    invoked in any of the three cases (the live proof the gate is genuinely capability-driven, not
    raw-feature-driven, mirroring item 55/PR 19's own most-important live-read test; a contrasting
    "tier modern → the same canary IS invoked" case proves the canary mechanism itself can catch a real
    call); fallback mutual exclusion (two overlapping same-name calls proven to run strictly one at a
    time via a shared log asserted to read exactly `"1 start,1 end,2 start,2 end"`, never interleaved);
    fallback independence (two different-name calls proven to genuinely interleave); fallback
    resilience (a first call whose `fn()` rejects — one variant via a rejected Promise, a second via a
    synchronous throw — does not prevent a second queued same-name call from running afterward, and the
    first call's own caller still receives the exact original rejection, never silently swallowed); and
    the queue-cleanup guarantee (the debug pending-count reads 0 once a name's queue fully drains,
    proven for both one call and a longer 5-call chain, and proven to read back UP while calls are
    still in flight, so the assertion genuinely exercises the map shrinking rather than a
    permanently-zero stub). **Proven load-bearing by breaking 4 representative guarantees one at a
    time** in the working tree — a raw-feature-only gate ignoring tier (3 "single most important test"
    failures + 2 source-level checks); removing the fallback's chain-onto-the-prior-tail line (1
    failure, exactly mutual exclusion, log read `"1 start,2 start,2 end,1 end"`); a failure's own
    promise becoming the queue's new tail instead of a settled-regardless-of-outcome derivative (jammed
    the queue exactly as predicted, observed directly); removing the cleanup `delete` line (exactly the
    2 queue-cleanup assertions) — then reverting each and re-confirming a clean 55/0 every time. One
    pre-existing item 55/PR 19 assertion (`shared_js_exports_capabilities_off_vw`) had assumed
    `capabilities: _capabilities` was the LAST key in the `VW` assembly, immediately followed by the
    closing `}` — legitimately broken by this PR's own `locks:` key appended right after it; fixed by
    widening that regex's closing delimiter to accept either `}` or `,`, preserving the actual
    guarantee it exists for; re-confirmed clean (17/0) afterward. `rps_lint.py` clean. See
    `CHANGELOG.md` `[1.74.0]`.
...
57. **`[1.75.0]` — `VW.workspace`: IndexedDB storage migration (multi-window support, PR 21/25, stage
    ...
    start handling a Promise or callback in this PR — so this cannot be "the same functions, reading
    IndexedDB now," it has to be a synchronous cache with IndexedDB underneath it. **The resolution:** a
    synchronous in-memory cache (`_wsCache`), bootstrapped instantly from a plain `_wsRead()` the moment
    any workspace function is first called on a page (so even the very first synchronous call sees a
    result that really did come from a valid read), with IndexedDB reconciled in afterward,
    fire-and-forget: if IndexedDB already holds records, it replaces the cache wholesale and becomes
    authoritative; if it reads back empty, a ONE-TIME migration writes the localStorage-sourced cache
    into it (localStorage's own key is deliberately left untouched afterward, never cleared — a frozen,
    unread backup costs nothing and turns a hypothetical rollback into "reads the last-known-good copy
    again" instead of data loss). From then on every mutation updates the cache synchronously and fires
    an async, best-effort write-through to IndexedDB — never also writing to localStorage again, which
    is exactly what lets a payload too big for localStorage's own quota succeed. Any IndexedDB failure
    degrades silently at the persistence layer only, EXCEPT a *repeated* failure streak (3 in a row),
    which fires one one-time toast on this file's own R13 "fail loud enough to be seen" discipline —
    past that point the tab's data genuinely only lives in memory, a real invisible data-loss risk, not
    a transient hiccup the cache already absorbs. **Which backing to use is decided once, at bootstrap,
    and latched for the page's life** — the one deliberate departure from "gate on the live
    capabilities getter" per call, since `VW.capabilities`'s own fields can flip once `RPS.mode`
    resolves from its `"modern"` default to the real tier after this file has already loaded, and
    re-deriving the backing on every call would risk stranding an already-committed cache entry against
    a resumed localStorage-only path that never saw it. **The one real limitation stated plainly:** two
    tabs open at once on modern tier each bootstrap their own cache independently and reconcile against
    IndexedDB asynchronously — a real, narrow window exists where the two can briefly disagree before
    both round trips finish, and this PR adds no new cross-tab live-sync mechanism to close it (item 2's
    ...
    broadcast onto the async cache-replacement step: a receiving tab cannot tell its own reconcile
    already finished from not-yet, so acting on such a broadcast could paint a still-bootstrapping view
    ...
    46's `test_a2_popout.py` coupling hazard, confirmed by a source-offset assertion. New
    `engine/tests/test_vw_workspace_indexeddb.py` + `engine/tests/js/test_vw_workspace_indexeddb_node.js`
    — the `.py` wrapper runs 24 static source-level checks plus a `node --check` gate and the node
    suite's own rollup (26/0); the node suite itself carries **43 real assertions** against a
    hand-rolled mock IndexedDB (genuinely async via `setTimeout`, driven only through the real
...
58. **`[1.76.0]` — `VW.workspace`: schema-versioned saved data (multi-window support, PR 22/25, stage
    6).** Depends on item 35/PR 2 (CRUD, already merged) ONLY — explicitly INDEPENDENT of item 57/PR
    ...
    shape. Fulfills the `schemaVersion` deferral item 47/PR 3 named explicitly at the time
    ("deliberately out of scope... `schemaVersion` and migration-on-read (Stage 6)"). A new
    `_WS_SCHEMA_VERSION = 1` constant is stamped onto every NEWLY-created record by `workspaceCreate()`;
    migrate-on-read runs at the ONE shared chokepoint item 57/PR 21 itself established
    (`_wsAllForRead()`/`_wsAllForMutation()`), never duplicated per-consumer. **The three cases, and the
    one shared decision function (`_wsClassifyRecordSchema`) both the read path and the import path
    call — never a second, independently-typed copy of the comparison:** schemaVersion MISSING entirely
    (every record PR 2-through-21 ever saved, the overwhelmingly common real case) is stamped with the
    current constant in place and written back; present and `<= _WS_SCHEMA_VERSION` passes through
    untouched; present and `>` (a technician's cached-newer build, or a rolled-back deploy against
    already-upgraded data) is CLEAN REFUSAL — excluded from `list()`'s returned array (matching
    `_wsItems()`'s own established "drop what's invalid, keep the rest" precedent) while every OTHER
    valid record still comes back correctly, and NEVER deleted or mutated in the underlying stored array
    (a rolled-back build must not destroy data a newer build already wrote); `get(id)` keeps its
    existing null-for-not-found convention but makes the two cases DISTINGUISHABLE via a `console.warn`
    naming the id and the unrecognized version plus a new debug accessor,
    `VW.workspace._lastGetSchemaRefusal()` (`_lastReadSchemaRefusals()` mirrors this for `list()`) —
    deliberately NOT part of the documented public shape, same convention as item 56/PR 20's
    `_debugPendingCount`. **Two real correctness hazards found and fixed during this PR's own
    load-bearing verification, not shipped blind:** (1) an eager write-back on EVERY migrated read would
    have silently defeated `_wsRead()`'s own pre-existing, deliberate "a read never rewrites
    corrupt/hostile storage" guarantee (item 35/PR 2's own test) whenever a migratable valid record
    shared storage with genuine junk — fixed by a new `_wsLastReadHadJunk` signal that defers the
    durable write specifically in that mixed case (the stamp still applies in-memory for that call's own
    return value; only the persist waits for a real mutation or a later clean read); (2) an eager
    write-back on the IndexedDB-backed path, fired before item 57/PR 21's own bootstrap/reconcile had
    resolved, could open a second out-of-band connection and write the still-provisional
    localStorage-sourced bootstrap cache into IndexedDB before reconcile's own read of it — making
    reconcile wrongly conclude "already migrated" instead of replacing the cache with
    genuinely-authoritative prior-session data; fixed by gating the eager commit behind `_wsIdbDb`
    already being an established connection (reconcile runs at most once per page, so once set, no
    bootstrap decision is still pending). **Export/import (item 47/PR 3):** `_wsExportPayload()` now
    also carries `schemaVersion`; `_wsValidateImportShape()` calls the SAME `_wsClassifyRecordSchema()`
    before `workspaceCreate()` is ever reached, so a future-schemaVersion import file is refused with
    the identical specific-`Error` convention PR 3's other malformed-import cases already use, and
    nothing is written to storage — an old (schemaVersion-less) export still imports cleanly, matching
    the read path's own "missing is fine" rule. New `engine/tests/test_vw_workspace_schema_version.py` +
    `engine/tests/js/test_vw_workspace_schema_version_node.js` — the `.py` wrapper runs 13 static
    source-level/placement/ES5 checks plus a `node --check` gate and the node suite's own rollup
    (15/0); the node suite itself carries **47 real assertions** against the real exported
    `VW.workspace` functions on the localStorage backing (deliberate, per this PR's own
    IndexedDB-independence): a fresh `create()` stamping the constant PROVEN IN ISOLATION (checked
    against raw storage before any `get()`/`list()` call could itself migrate-and-mask a missing
    stamp); a hand-constructed old-shaped fixture (no `schemaVersion` field, exactly real PR2-21 data)
    read correctly via `list()`/`get()` AND durably re-stamped in the backing store afterward; a
    ...
    Item 35/PR 2's, item 47/PR 3's, and item 57/PR 21's OWN original test suites were re-run against
    ...
    from a masking migrate-on-read; the missing-schemaVersion stamp/write-back removed: 3 failures; the
    ...
    commit; export dropping `schemaVersion`: 2 behavioral + 1 structural failure; import skipping the
    shared classifier: 4 behavioral + 1 structural failure; a hand-rolled second schemaVersion
    comparison added alongside the shared call in the import path: 2 behavioral + 1 structural failure,
    exactly the "not duplicated" guarantee). `rps_lint.py` clean (the same backtick/ellipsis/"async "
    false-positive class this same multi-window session already hit in earlier PRs, reworded rather than
    suppressed). See `CHANGELOG.md` `[1.76.0]`.
59. **`[1.77.0]` — `VW.workspace`: File System Access API for export/import (multi-window support, PR
    23/25, stage 6).** Depends on item 47/PR 3 (`exportFile`/`importFile`, already merged) and item
    55/PR 19 (`VW.capabilities.fileSystemAccess`, already merged) — both reused directly, per the
    ...
    `VW.workspace`, both gated on the SAME `_capabilities.fileSystemAccess` item 55/PR 19
    ...
    item 47/PR 3's and item 58/PR 22's own validation/migration path, never a second,
    ...
    button ADDED next to the existing "📁 Import from file…" control — both visible/enabled only
    when `VW.capabilities.fileSystemAccess` is true, both genuinely wired to the two new functions
    above (never an orphaned API addition nobody can reach). The existing download link, the
    ...
    item 58/PR 22's own classifier produces (not a second, hand-rolled comparison); a successful
