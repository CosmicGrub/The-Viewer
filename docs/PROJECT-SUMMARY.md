...
nav's ↗ pop-out links — the first real UI consumer of any of it (`[1.55.0]`, `[1.54.0]` reserved by
a sibling responsive-baseline PR not yet merged) — and `VW.bench`, which promotes the
twice-duplicated "My Bench" accessor into `shared.js` and makes the pinned list live-sync across
tabs — the first change in that initiative a technician can actually see repaint on its own
(`[1.56.0]`) — see the reconciliation notes below and §8 items 25–38). This document +
