...
RUN_MODE_LABELS = {"auto": "Auto (recommended)", "performance": "Performance",
                   "retro": "Retroactive Post-Support"}
...
def feature_flags(mode):
    """Return the on/off switches + tuning for a mode. The UI reads `effects`/`animations`/`default_dpi`/
    `loupe`/`polyfills`; the server reads `page_cache`/`prefetch`/`sqlite`/`render_dpi_cap`."""
    if mode == "modern":
        return {"mode": "modern", "effects": "full", "animations": True, "default_dpi": 150,
    ...
    if mode == "lite":
        return {"mode": "lite", "effects": "lite", "animations": False, "default_dpi": 120,
                ...
                "sqlite": {"cache_kb": -2000, "mmap": 0, "temp_store": "FILE"}}
    # legacy
    return {"mode": "legacy", "effects": "lite", "animations": False, "default_dpi": 100,
            ...
            "sqlite": {"cache_kb": -1000, "mmap": 0, "temp_store": "FILE"}}

def profile_summary(profile, mode, reason):
    p = profile or {}
    return {"mode": mode, "reason": reason, "flags": feature_flags(mode),
